...
//!
//! Time, randomness, scheduling, and the wire all sit behind traits
//! ([`Clock`], [`Entropy`], [`Spawner`], [`Transport`]). The simulator
//! (`actor-simulation`) supplies deterministic implementations; this crate
//! supplies the production counterparts, so a [`ClusterSystem`] runs for real
//! across processes and hosts with nothing else in the framework changed
//! (spec §4.6):
//!
...
//! - [`FileRaftWAL`] — a voter's durable Raft state on the local
...
// This crate IS the host seam (spec §4.6, §18.1): the one place allowed to read
// the wall clock, spawn OS/tokio tasks, and seed from the OS. The workspace
// `clippy.toml` forbids those host APIs everywhere else to keep the simulation
// build deterministic; this allow is the determinism boundary.
