...
async fn serve_connection(harness: Harness<TcpCluster>, stream: tokio::net::TcpStream) {
    ...
    while let Ok(Some(line)) = lines.next_line().await {
        ...
        // ops resolve to a single reply. It rides a grain record subscription
        // (granary §7.9) — pushed, not polled.
        if let Op::Watch {
            kind,
            session,
            turn,
        ...
        {
            tokio::spawn(watch(harness, tx, request.id, kind, session, turn, from));
            continue;
        }
...
/// One subscription (`Op::Watch`): stream a run's records to the client as they
/// commit. Rides a harness [`Follower`] (a grain record subscription, granary
/// §7.9) — pushed, not polled — and stops when the watched turn's `RunEnded`
/// record arrives. The follower reconciles by seq and backfills across a leader
/// move (granary G16), so the client sees the committed sequence exactly once.
...
async fn watch(
    ...
    let session_ref = harness.session(&kind, SessionId::new(session));
    let mut follower = session_ref.follow(Seq::new(from));
    loop {
        match follower.next().await {
            ...
            // A real durability outcome (the shard cannot serve right now):
            // report it and end the watch. The run continues on the cluster; the
            // adapter's prompt reply still drives completion, and a fresh watch
            // re-attaches.
            Err(e) => {
                let _ = tx.send(Response {
                    id,
