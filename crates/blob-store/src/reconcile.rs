...
//! The same pass reclaims tombstones whose retention is no longer needed (spec
//! §5.3).
...
/// that does not answer promptly is skipped this round rather than stalling B6
/// re-replication behind it.
...
async fn reconcile_pass<S: BlobSystem>(inner: &Inner<S>) {
    ...
    // Without this, a healed `unreachable` holder resurrects the namespace it never
    // learned was gone. It runs *after* re-replication so one slow or dead peer never
    // starves B6; while gossip converges, the push vector stays guarded by the
    // receiving owner's `StoreAck::Deleted` reject.
    ...
    // the tombstone set is retained: cluster-wide set GC is not implemented.
...
/// in-memory awareness set, and finally emitted so the simulator's checkers observe
/// the node becoming aware. Peers are visited in a stable (sorted) order, and only
/// genuinely new tombstones mutate state, so the pass is idempotent and
/// seed-reproducible.
async fn sync_tombstones<S: BlobSystem>(inner: &Inner<S>, members: &[actor_core::NodeId]) {
    ...
    // Pull every peer concurrently, so a single slow or dead peer does not serialize
    // the pass behind it. The learned tombstones are applied below in the
    // deterministic peer order, so the emitted event stream stays seed-reproducible
    // (spec §8).
    let pulls = peers
        .iter()
