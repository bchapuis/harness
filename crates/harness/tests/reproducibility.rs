...
//! including harness events — with model, journal, and sandbox faults firing
//! under seed control; and a swarm sweep holds the H-invariants across seeds
//! while those faults flow.
...
/// A workload that runs several sessions through the tool loop with seeded
/// model failures and journal latency/outages (§12.2). Callers tolerate
/// honest failures — a model error that survives the policy is a journaled
/// terminal outcome, not a hang — and re-submit on transport errors, the H3
/// contact discipline.
struct FaultedWorkload {
    name: &'static str,
    sessions: usize,
    model_fail_num: u64,
    journal_fail_num: u64,
    /// Coverage accounting (§11): faults the sweep actually fired while
    /// agent traffic flowed, aggregated across seeds.
    model_fired: Arc<std::sync::atomic::AtomicUsize>,
    journal_fired: Arc<std::sync::atomic::AtomicUsize>,
}
...
impl FaultedWorkload {
    fn new(
        ...
        FaultedWorkload {
            ...
            model_fail_num,
            journal_fail_num,
            model_fired: Arc::new(std::sync::atomic::AtomicUsize::new(0)),
            journal_fired: Arc::new(std::sync::atomic::AtomicUsize::new(0)),
        }
    }
}

impl Workload for FaultedWorkload {
    ...
    fn run(&self, system: SimSystem) -> BoxFuture<'static, ()> {
        ...
        let mut journal = FaultedJournal::new(
            InMemoryJournal::new(),
            system.clock().clone(),
            system.entropy().clone(),
        )
        .with_latency(Duration::from_millis(50))
        .with_failures(self.journal_fail_num, 10);
        journal.fired = Arc::clone(&self.journal_fired);
        ...
        boxed(async move {
            ...
            for i in 0..sessions {
                ...
                waiting.push(async move {
                    ...
                    // TurnId on a reported transport failure, bounded.
                    for _ in 0..20 {
...
fn the_event_stream_reproduces_byte_for_byte_per_seed() {
    // The determinism contract over the real stream (core spec §18.1 #1),
    // harness events included: one leak — a wall clock, an OS thread, an
    // unseeded draw — breaks this, which is the point.
    for seed in [3, 17, 5151] {
        let faultless = FaultedWorkload::new("repro-faultless", 3, 0, 0);
        check_reproducible(&faultless, seed).expect("faultless run reproduces");
        let faulted = FaultedWorkload::new("repro-faulted", 3, 1, 1);
...
fn the_h_invariants_hold_across_a_faulted_seed_sweep() {
    let workload = FaultedWorkload::new("harness-swarm", 4, 1, 1);
    run_swarm(&workload, 1..=25).expect("every seed upholds the H-invariants");
    // Coverage accounting (§11): a green sweep proves model and journal
    // faults actually fired while agent traffic flowed, not that seed luck
    // dodged them.
    assert!(
        workload
            .model_fired
    ...
    assert!(
        workload
            ...
            .load(std::sync::atomic::Ordering::SeqCst)
            > 0,
        "the sweep never injected a journal fault"
    );
}
