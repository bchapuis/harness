...
//!
//! These are scenarios: each proves a fault behaves correctly *when it fires*.
//! That every fault type fires at all across a seed sweep — drops, duplication,
//! delay/reorder, partition/crash, and registry outage/staleness — is the
//! companion claim, and it lives in `conformance_faults_swarm.rs`.
...
fn per_pair_fifo_survives_latency_jitter() {
    ...
    sim.block_on(async move {
        ...
        let remote = node_b.resolve::<Order<SimNode>>(order.id().clone());
        for i in 0..10 {
...
fn total_loss_yields_timeout_not_a_hang() {
    ...
    let outcome = sim.block_on(async move {
        ...
        node_b
            .resolve::<Greeter<SimNode>>(greeter.id().clone())
            .ask_timeout(
                Greet {
                    name: "world".into(),
                },
...
fn duplication_is_tolerated_with_one_outcome_at_the_caller() {
    ...
    let count = sim.block_on(async move {
        ...
        let remote = node_b.resolve::<Counter<SimNode>>(counter.id().clone());
