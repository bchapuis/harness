//! Conformance: fault-injection **coverage** across a seed sweep (spec §18.3).
//!
//! The scenarios in `conformance_faults.rs` prove each fault behaves correctly
//! when it fires. These prove it *fires at all*: a swarm that configures faults
//! but, by seed luck, never triggers one gives false confidence, so the cluster
//! swarm tallies what it actually exercised and these tests assert every fault
//! type fired at least once across the declared range.
//!
//! Split from the scenario file because these are sweeps: they run at
//! `coverage_seeds` width, fail by naming a seed rather than a sequence, and by
//! convention sweeps live in a `*swarm.rs` binary (docs/simulation-testing.md).
...
fn the_cluster_swarm_actually_exercises_every_fault_type() {
    ...
    let stats = match run_cluster_swarm_coverage(&workload, coverage_seeds(0..64)) {
        Ok(stats) => stats,
