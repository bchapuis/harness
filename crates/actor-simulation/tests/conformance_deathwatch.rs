//! Conformance: death watch (spec §12): multiple watchers, `unwatch`, and signal
//! ordering through the mailbox, plus a watcher receiving exactly one
//! [`Terminated`] when a watched actor stops, fails, or — the distributed case —
//! has its node declared `down` (the node-down cascade, spec §8.1 step 4).
...
impl Message for WatchStop {
    type Reply = ();
    const MANIFEST: Manifest = Manifest::new("watch.Stop");
}

impl Handler<WatchStop> for Target {
    async fn handle(&mut self, _msg: WatchStop, ctx: &Ctx<Self>) {
        ctx.stop();
    }
}
...
impl Handler<Terminated> for WatchWatcher {
...
fn local_watch_yields_terminated_once_on_stop() {
    ...
    sim.block_on(async move {
        ...
        let _watcher = node.spawn(WatchWatcher {
            target: target.clone(),
            got: recorded,
        });
        // Let the watcher's `started` register the watch before the target stops.
        clock.sleep(Duration::from_millis(1)).await;
        target.tell(WatchStop).await.unwrap();
    });
...
fn remote_watch_after_death_reports_the_true_reason() {
    ...
    sim.block_on(async move {
        ...
        let _watcher = node_a.spawn(WatchWatcher {
