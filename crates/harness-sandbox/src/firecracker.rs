...
//! the guest speaks to a virtual machine monitor, not the host kernel. The
//! price is provisioning cost (a boot per activation; snapshot warm pools
//! are not implemented, sandbox spec §7) and a Linux + `/dev/kvm` host at
//! runtime — this module *builds* everywhere, and its provisioning path
//! fails as a `ToolError` outcome where KVM is absent.
//!
...
//! so the bind-mount of the docker fallback has no analogue. Every call
//! brackets the command with a synchronization pair:
//!
...
//! client; the guest process runs on until release), and native calls need a
//! tokio runtime.
...
/// Cap on any single non-tar (JSON) frame: the bound on a malicious or
/// corrupted frame header.
const MAX_FRAME: usize = 1024 * 1024;

/// How long provisioning waits for the guest agent to answer a ping.
...
/// [`crate::TieredSandboxes::with_firecracker`]: the node's VMM assets. Which
/// base rootfs a session boots is solely the profile's `image` (harness spec
/// §5.3 item 4), a host path to an ext4 containing `/sbin/fc-agent`; keeping
/// it out of this config keeps the choice inside the digest the cluster
/// agrees on.
#[derive(Clone, Debug)]
...
pub(crate) struct FirecrackerTier {
    ...
    /// The profile's `image`: the base-rootfs path a session boots.
    image: String,
    ...
    /// before touching any tokio API when no Native call ever ran.
...
/// Connect to the guest agent: the shared muxer handshake, at [`VSOCK_PORT`].
