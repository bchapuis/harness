...
The revision-varying behavior in that sweep is the *workload's own*, not the tree's, because no boundary here has a second revision yet. That is the honest limit of what it proves: the send-side gate is usable and a build that ignores it is caught, but nothing in `crates/` is itself kept behind a revision, and no granary invariant or linearizability check is asserted across the transition. Those remain open, and both are now ordinary work rather than blocked work.
