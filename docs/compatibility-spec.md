...
- **The Raft log's application payloads** — two of them, in the same position and with the same gap. `ShardMapCommand` (granary §7.6) is `serde_json` and `MembershipCommand` (actor §9.4.3 item 1) is a fixed 9-byte tag-plus-node form, both inside `EntryPayload::App`, both riding the already-stamped `actor.raft.log`. That log's revision covers the **envelope**, not the payload, so neither has a boundary of its own.

  Both are **fail-closed** without one: a node that cannot decode a committed payload stops applying that log rather than skipping an entry its peers will apply, which is what keeps its state a prefix of the log rather than a gap in it (granary §7.6 and actor §9.4.3 item 1 each carry the divergence that skipping produced, and why stopping is safe where skipping was not). What a boundary would add is a **name** for the refusal. Today a build that meets a payload from a later build stops with a parse failure or an unknown tag, indistinguishable from corruption; a `granary.shardmap` and an `actor.membership` row in §3, a revision ahead of each payload, and a corpus fixture apiece would make it stop on *"revision 2, and I accept 1"* instead — the difference between an operator reading a mixed-version rollout and reading a damaged log. Believing either refusal end to end needs the mixed-version machinery of §4 rather than a corpus fixture, which is why this is not format work.

  The membership payload sits second of the two, because stopping costs it less. Every transition a build knows also travels in **gossip**, stamped with the commit index that decided it, so a peer that applied the entry can still deliver the outcome to a node whose log no longer can (actor §9.2) — a repair path the shard map has no equivalent of. That repair covers exactly the transitions the stopped build can name, which excludes the one that stopped it.
