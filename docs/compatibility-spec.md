...
| `granary.shardmap` | 1 | `GRSMAP` and a `u16`, ahead of the JSON payload in each committed `EntryPayload::App` | enum variants in the payload; the envelope by revision | [granary](granary-spec.md) §7.6 |
| `actor.membership` | 1 | `ACMEMB` and a `u16`, ahead of the tag-plus-node payload in each committed `EntryPayload::App` | new tags in the payload; the envelope by revision | actor §9.4.3 |

**The `Rev` column is the newest revision *defined* at a boundary, which is not always the one a build *writes*.** The two differ for exactly as long as a bump is in flight, because **V4** widens the accepted range in one release and moves the written revision in a later one. The column tracks the definition rather than the window, because that is what a corpus must hold — a revision nothing has bytes for is a revision nothing can be shown to still read (§4) — and the window itself is stated where it lives, in the boundary's `Window`. **No row is mid-flight today**: every boundary's revision is the one its build writes, and every build reads what it writes (**V3**).

The last two rows to be registered, `granary.shardmap` and `actor.membership`, are payloads riding the already-stamped `actor.raft.log`, whose revision covers the envelope and not them — which is why each needed a boundary of its own rather than a bump of the log's. With them registered, no durable byte sequence in the tree is unstamped except the two content-addressed ones below, which are stamped by their names.

**Introducing a boundary is not a bump, and it did not take V4's two releases here.** Read-new-first buys one release in which every peer learns to read a form before any peer writes it, and it earns its cost exactly when there are peers in the field on the older build. This tree has no deployment to roll: a build of it meets only its own peers, so both boundaries landed reading and writing the stamp in one step. **V4 still binds every future change**, and it binds the *first real one* — the moment a deployment exists whose upgrades matter, or a boundary takes a second revision, the two-release ordering is the rule again and the reason it was skipped here no longer holds. What did *not* get skipped is **V5**: both decoders still adopt their unstamped predecessor, because that obligation is about bytes at rest rather than release cadence, and a log written by an earlier build of this tree is still on disk.
...
This is a deliberate position rather than an unfinished one, and it was reached by building the other one first. **Decided 2026-08-07: build the machinery, not the revision.** A working `actor.wire` revision 2 was built end to end and pulled back — a gated frame appended to `Frame`, a `v2.bin` corpus fixture, §5's cluster-wide minimum folded from it, and both sweeps carrying the property over a real format. A second pass then removed the *seams* it left behind: a `Frame::revision` returning a constant, a receive-side refusal only a contrived test could fire, a `Transport::local_window`, a catalogued invariant that could never trip, and a granary rolling sweep that largely duplicated the restart sweep beside it. Roughly sixty lines of inert production code across five files, one call of it on the path every inbound frame takes.

The reasoning both times was the same. A revision invented to have something to exercise is a revision every later build carries under **V5**, for a rollout nobody is performing; the tree has no production deployment to de-risk. And a seam that cannot fire until that revision exists is not insurance, it is maintenance. What is built ahead of need is therefore only what costs nothing until used — the window type, the corpus and its completeness gate, `Transport::peer_version`, `SimNetwork::set_wire_window` and the `Rollout` vocabulary — so that the release which first changes a format inherits them rather than writing them under the pressure of the change. Machinery that would have to be *maintained* in the meantime is deferred with the revision it serves (§5).

One test was kept from that work because it earns its place without a second revision: `frame_discriminants_are_stable` (§3.1), since `actor.wire`'s corpus pins the `Hello` preamble and nothing else pins the frame vocabulary's layout.

**What would reopen this:** a format change something actually needs, or a deployment whose upgrades are worth rehearsing against. That the build is *possible* is not new evidence — it has been done once, and this paragraph exists so it is not attempted a third time. The work left at that point is small, because the seams are in place. For a wire bump: widen `WIRE` to `Window::new("actor.wire", 1, 2, 1)` in one release and flip to `…, 2, 2)` in a later one (**V4**); append the new variant rather than editing one and extend `frame_discriminants_are_stable` to pin it; gate the send on `Transport::peer_version`, remembering that `None` means *not yet known*; give a simulated node a way to answer with its *own* window rather than the build's constant, or the sweeps cannot roll one binary through releases; and check in the new revision's corpus fixture beside `v1.bin`. The `Rollout` vocabulary and the sweeps that drive it need no change. A **durable** bump needs one piece that does not exist: a rollout changes what a node sends, where a durable revision changes what is already on disk, so the workload must write records under one revision and read them back under another — a restart onto a widened window, not a nemesis action.

---

## 5. Deferred

**Every format in the tree is stamped**; §3's registry is complete, and what is left here is *mechanism above the stamps* rather than a boundary still missing one. Both entries wait on the same thing — a boundary with a second revision — and neither can be exercised before one exists.

One thing is **not** deferred, and is recorded here because a reader arriving at this section will otherwise look for it. The last two unstamped payloads, `granary.shardmap` and `actor.membership`, now have boundaries and are read and written stamped (§3). A **restamp** is a different obligation from a **stamp**, which is why the first entry below could wait and those two could not: a stamp goes on bytes that outlive the build that wrote them, so it has to be there before it is needed, while a restamp corrects a header the moment a window widens and can be added the same day that window does.
...
- **Raising a log's record-schema stamp in place** — a `Wal` appends frames at the revision its build writes without updating the header, so once a caller's window spans two revisions the stamp understates until a compaction restamps it (wal §2.1 rule 5). Fail-closed, and a caller can work around it by compacting; removing the caveat means a second, non-append handle to rewrite two bytes of the header on the first append after a bump.
  ...
  Deferred until a caller needs it. All three record-schema windows in the tree — `actor.raft.log`, `granary.store.manifest`, `granary.store.segment` — are a single revision wide, and a one-revision window cannot understate, so the fix today is a branch on the append path that no build can reach, on the hottest durable path this tree has. That is the shape §4's last paragraph declines in general.

  Waiting also leaves room to settle what the fix costs, which is not obvious, and **the direction has to be settled before any of it is written**. One header field answers for a file whose records span two revisions, and neither answer serves every reader: understating refuses a build that could have read the file, while raising the stamp admits a build that has since dropped the lower revision and lets it misparse the older frames — unsafe in a way today's behaviour is not. Raising it is therefore sound only if **narrowing** a window is first gated on compacting the affected logs — the mirror of wal §2.1 rule 5's advice for widening, which is written for the other direction and does not cover this. Establish that rule, or a way to record more than one revision per file, before touching `append`. Today's understating stamp is the conservative half of the trade, which is the second reason it is not urgent to change.

  The ordering is then the whole of the work: the header must claim the higher revision, durably, *before* a frame at that revision lands, or a crash between the two leaves exactly the understating file this fixes. A `Wal` must also hold the *stamped* revision alongside the one its build writes, which it does not today.
- **A cluster-wide minimum revision** — carrying each member's announced range in the membership digest, so a behavior can enable itself only once the whole cluster accepts it. This turns **V4** from a policy into a mechanism.
  ...
  **Built once and pulled back, 2026-08-07**, together with the `actor.wire` revision 2 that would have given it a caller (§4). It has none until a boundary has a second revision to gate on, and a mechanism that cannot fire is maintenance rather than insurance. What it would take is written down so the next attempt is not a rediscovery: carry each member's announced range in the SWIM gossip digest (`MemberDigest`, which already rides `Ping`/`Ack`/`PingReq`), or in a frame of its own if the digest's riders make a positional layout change too expensive, and expose the cluster-wide minimum as something a caller can gate on. A node that has not yet heard from a member must read the minimum as *not yet known* and write conservatively, never as *no constraint* — the hazard §3.1 states for `peer_version`. And the trap worth reading §3.1's design note about before starting: **only first-hand evidence can retract**, so a node that rolls back runs a build that cannot announce its own retraction, and a belief built from announcements alone stays wrong until something narrows it from the association.
