...
**Status:** Draft v1
**Scope:** How a format in this workspace declares which revision it is, which revisions a build accepts, and what happens outside that range. This document owns the *compatibility vocabulary* and the **registry of boundaries** (§3). It does not own any format: each format's layout stays in the spec of the crate that writes it.
...
> **Design stance.** Every format in the tree is produced by one build and consumed by another — across the network during a rolling upgrade, and across time on disk, where a journal written today is read by next quarter's binary. At each such place one question decides whether the system is safe: can the reader tell what it is looking at, and does it refuse when it cannot? A **misparse** is the failure worth engineering against, because it converts a version skew, which an operator diagnoses in a minute, into silent corruption, which they cannot. So the machinery is small and it lives in one place: a revision, a range, and a named refusal. What it deliberately does *not* try to be is a schema-evolution framework. Field-level compatibility rules tell you that v1 bytes parse in v2; they say nothing about what v2 should *do* with them, when it is safe to start writing v2, or whether v1 can still read what v2 wrote. That work is the same under any encoding, and this document is about making it explicit rather than automatic.
...
A boundary needs exactly three things, and a boundary that has all three needs nothing else:
...
**V5 counts releases, not revisions.** An earlier draft of this document wrote the support window as "accepts *N−2..N*", which cannot be implemented: *N* there is a release number, while a revision is per-boundary and does not advance when a release ships that leaves the format alone. A boundary sitting at revision 1 across ten releases has no "N−2". The obligation is a *duration* — how long a retired revision must still be readable — and the ranges in this tree are mostly a single revision wide precisely because nothing has been retired yet.
...
The `compat` crate is the vocabulary and nothing else. It owns no format, reads no file, and depends on nothing else in the tree, so it can sit below `wal` — which is itself below its callers ([wal](wal-spec.md) §1) — and every boundary in the workspace can share one definition of "compatible" and one wording for the refusal.
...
- **`Stamp`** — a magic and a revision written ahead of a body, for a boundary with bytes at rest. The stamp sits **outside** the body's own encoding, which is the point: a positional format such as `postcard` cannot version itself, having no field name to make optional and no discriminant to grow, so reading the revision before any decoder runs is what makes refusal-instead-of-misparse possible. The magic travels with the window in one value, so a writer and a reader cannot disagree about it.
...
- **`negotiate`** is two-sided, for a live peer: the highest revision both ends accept, or a refusal when the ranges are disjoint. Both ends compute it from the same two ranges and so reach the same answer, which is why a handshake needs no confirmation round — each side announces, each side decides, and they cannot disagree.

---

## 2.1 Negotiated and durable boundaries need different things

The two verdicts are not merely two call shapes; they mark the division that governs how a format at each kind of boundary should be built.

A **negotiated** boundary — the association handshake — can afford a strict format. Both ends are present at the moment they speak, each states what it accepts, and either may refuse. Nothing has to be guessed, because the counterparty is available to be asked.

A **durable** boundary has no counterparty. Its reader is a build that does not exist yet, and by the time that reader runs, the writer is gone. There is no negotiation to fall back on: the bytes must carry everything needed to interpret them, or be refused. Under a positional encoding such as `postcard` — no field names, no discriminants to grow — that means every added field is a new revision, with a second decoder to write and to keep. That cost is not paid once; it is paid at every future change, and a format expensive enough to change is a format people work around instead, which is how a system accumulates the parallel side-tables and "temporary" adjunct files that outlive everyone who understood them.

So a durable envelope MUST carry an **extension area** (`compat::Extensions`), and the criticality rule is what makes it sound rather than a licence to misread:
...
This is PNG's ancillary/critical chunk rule. The division of labour is the point: a **revision** says *these bytes are a different format*; an **extension** says *these bytes are the same format, carrying more*. A change that reinterprets bytes already defined is never an extension, however tempting — it is a revision.
...
**Where this does not apply.** Not every durable format needs an area of its own, and adding one where a cheaper mechanism already exists is waste:
...
**A decoder MUST admit the area before using the body.** The check is a call (`Extensions::admit`) rather than something the encoding enforces, so skipping it compiles and yields precisely the silent misread the criticality bit exists to prevent — a critical entry ignored. Every boundary with an area names its known keys next to its window, and the golden corpus (§4) is what should catch a decoder that forgets.
...
**Granularity is a cost decision.** The right granularity for a stamp is the coarsest one that still catches the error, because a stamp's cost is paid per stamped item and its value is paid once per mistake. The boundaries above span the range: an association is stamped once for its whole lifetime and can afford a full announced range; a snapshot is large and cold, so eight bytes and a codec name are noise; a journal record is the hottest durable path in the tree and is stamped for **zero bytes** by reserving a bit of a byte it already carries. A uniform rule would be simpler to state and worse to live with.
...
The negotiated revision is currently **discarded** after the verdict, and the consequence should be stated plainly rather than left to be discovered: **this boundary cannot yet be bumped.** Negotiating a revision is only half of what a rolling upgrade needs. The other half is the *send-side gate* — a node that speaks revision 2 must not send a revision-2 frame to a peer that settled on 1 — and there is nothing to gate on, because the negotiated value reaches no caller. `Frame` is a serde enum, so a peer receiving a variant it does not know fails to decode and the association is torn down; negotiation lets that be *detected* early, not *avoided*.

What exists today is therefore the refusal, which is worth having on its own: it converts the first future bump from a cluster-wide mutual partition, which the previous equality check guaranteed, into a controlled rejection of one peer. What does not exist is the ability to run two wire revisions at once.

Closing this needs the negotiated revision surfaced to the cluster layer — `Transport::peer_version`, so the transport keeps owning negotiation while the layer above decides what to send. It is deferred (§5) rather than built because it has no consumer yet and would otherwise be a method that only relays; the first consumer is most likely the mixed-version simulation of §4, which cannot be written without it either. **Anyone planning a wire-format change should expect to build it first.**
...
The write-ahead log's header (wal §2.1) is the one place a single stamp is split between the crate that owns the *layout* and the caller that owns the *records*, and it shows what the split is for. `wal.frame` and `wal.checksum` are the crate's: they say how a frame is shaped and which digest closes it. `actor.raft.log`, `granary.store.manifest`, and `granary.store.segment` are the callers': one per log, because the logs evolve independently — adding a field to a segment op says nothing about a manifest entry.

The caller's field is the load-bearing one. `Wal<T>`'s records are `postcard`, so `T` cannot gain or reorder a field within a revision, and the revision has nowhere to live but outside the payload. `wal` stores it and returns it without interpreting it, which is what keeps one crate from having to know every caller's schema.
...
A magic and a revision ahead of the composite's `postcard` body (granary §7.12). The revision is admitted before the body is decoded, so a composite from another revision is refused rather than misparsed — the reason a positional encoding needs an *external* stamp at all.

The body also records the **codec that encoded facet 0's contribution**, which is the one part of a snapshot that is not codec-independent (granary §4.1, §5). A mismatch is refused naming both codecs. This is not a revision check and does not use `Version`: the codec is an identity, not an ordering, and nothing is gained by pretending otherwise. It is the coarse-grained half of a gap that a `granary.store` stamp (§5) would close completely; a snapshot carries its own copy because it is the one durable artifact that can travel between stores.

The body carries an **extension area** (§2.1). It is the boundary that most needs one: the composite is `postcard`, so without it a single added field — a compression marker, a provenance note, a per-facet digest — would be a revision bump and a second decoder kept for the life of the support window. One byte while empty buys that back.
...
**A golden corpus** — per boundary, checked-in bytes from each revision and a test that decodes every one with the current build — is what actually catches the failure a type system cannot see: adding a field to a `postcard` struct compiles cleanly and breaks every stored copy of it. *Not yet implemented.*

**Mixed-version simulation** — simulated nodes given *different* windows, prior record and message definitions kept behind their revision, and the granary invariants and linearizability checks asserted across a rolling upgrade and a rollback. This is the reason to route every boundary through one `Window` type, and it tests the thing that actually breaks in production. *Not yet implemented.*
...
- **`granary.store`** — a grain's **event payloads** are encoded with the deployment's codec, and nothing on disk records which codec that was. §3.3 closes this for snapshots, but a grain with records past its last snapshot — or none at all — is still unguarded, so swapping the codec turns those records into a storm of corrupt-grain activation aborts rather than one diagnosable configuration error. A store-level stamp is the coarsest granularity that catches every record and snapshot at once, and is worth having *before* a codec swap rather than after.
- **Sidecars** — the Raft term and snapshot pointers, the shardmap, and the blob-store tombstones are durable formats written through `wal::atomic_replace` with no stamp. `compat::Stamp` wraps them without changing the primitive, which keeps its opaque-bytes interface intact.
- **`Transport::peer_version`** — the negotiated wire revision, surfaced so the cluster layer can gate what it sends. Until it exists, `actor.wire` can refuse an incompatible peer but cannot run two revisions at once, which means it cannot be bumped (§3.1). First in this list's priority order, because it also unblocks the mixed-version simulation of §4.
...
- **A cluster-wide minimum revision** — carrying each member's announced range in the membership digest, so a behavior can enable itself only once the whole cluster accepts it. This is what turns **V4** from a policy into a mechanism.
