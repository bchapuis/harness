...
**The `Rev` column is the newest revision *defined* at a boundary, which is not always the one a build *writes*.** The two differ for exactly as long as a bump is in flight, because **V4** widens the accepted range in one release and moves the written revision in a later one. The column tracks the definition rather than the window, because that is what a corpus must hold — a revision nothing has bytes for is a revision nothing can be shown to still read (§4) — and the window itself is stated where it lives, in the boundary's `Window`. **One row is mid-flight today**: `granary.shardmap` is defined at revision 1 and has a corpus for it, and this build reads that form, but its writer still emits the *unstamped* predecessor and flips in a later release (granary §7.6). The shape is V4's, applied to a boundary's introduction rather than to a bump — the same two steps, the same reason for the gap between them — so the same reading of the column holds: it says what the boundary *is*, not what today's writer emits. Every other row's revision is also what its build writes.
...
- **The control group's membership transitions** — `MembershipCommand` (actor §9.4.3 item 1) is a fixed 9-byte tag-plus-node form inside `EntryPayload::App`, riding the already-stamped `actor.raft.log`. That log's revision covers the **envelope**, not the payload, so this has no boundary of its own. Its twin, `ShardMapCommand`, was the same gap and now has one (`granary.shardmap`, §3), which is the model: a magic and a revision ahead of the payload, an unstamped predecessor adopted as revision 1, and the writer flipping a release after the reader.

  It is **fail-closed** without a boundary: a node that cannot decode a committed transition stops applying rather than skipping an entry its peers will apply, which keeps its member set a prefix of the log rather than a gap in it (actor §9.4.3 item 1 carries the divergence skipping produced, and why stopping is safe where skipping was not). What a boundary would add is a **name** for the refusal — today an unknown tag is indistinguishable from a short or damaged payload, and both stop the node with the same message.

  It ranks below the shard map's for two reasons. Stopping costs it less: every transition a build knows also travels in **gossip**, stamped with the commit index that decided it, so a peer that applied the entry can still deliver the outcome to a node whose log no longer can (actor §9.2) — a repair path the shard map has no equivalent of, and one that covers exactly the transitions the stopped build can name, which excludes the one that stopped it. And a stamp costs it more: the payload is nine bytes today, so a magic and a revision more than double every membership entry ever written, where the shard map's JSON absorbs the same prefix without notice. Believing the refusal end to end needs the mixed-version machinery of §4 rather than a corpus fixture, which is why this is not format work.
- **Raising a log's record-schema stamp in place** — a `Wal` appends frames at the revision its build writes without updating the header, so once a caller's window spans two revisions the stamp understates until a compaction restamps it (wal §2.1 rule 5). Fail-closed, and a caller can work around it by compacting; removing the caveat means a second, non-append handle to rewrite two bytes of the header on the first append after a bump.
  ...
  Deferred until a caller needs it, which is the one entry here that waits on purpose rather than on effort. All three record-schema windows in the tree — `actor.raft.log`, `granary.store.manifest`, `granary.store.segment` — are a single revision wide, and a one-revision window cannot understate, so the fix is a branch on the append path that no build can reach. Unlike the payloads above, nothing is lost by waiting: a **stamp** must be written before it is needed, because the bytes outlive the build that wrote them, but a **restamp** corrects a header the moment a window widens and can be added the same day that window does.

  Waiting also leaves room to settle what the fix costs, which is not obvious. One header field answers for a file whose records span two revisions, and neither answer serves every reader: understating refuses a build that could have read the file, while raising the stamp admits a build that has since dropped the lower revision and lets it misparse the older frames. Raising it is therefore sound only if **narrowing** a window is gated on compacting the affected logs first — the mirror of wal §2.1 rule 5's advice for widening, which does not cover this direction. Today's understating stamp is the conservative half of that trade, which is the second reason it is not urgent to change.
