...
- **One substrate, many facets.** Durable features beyond the event fold — the KV map (§7.13), the workspace filesystem (§7.11), the SQLite database (§7.14) — are **facets**: interpretations over the two storage primitives (ordered records and immutable blobs), sharing one journal, one atomic commit per command, and one composite snapshot (§7.12). A new feature MUST NOT add a third replication mechanism.
...
| **Facet** | A durable storage feature of one grain — the event fold, the KV map (§7.13), the filesystem (§7.11), the SQL database (§7.14) — defined entirely as an interpretation over the two storage primitives: what its records mean, what it keeps in blobs, what it contributes to the snapshot (§7.12). |
...
**Facets stage; the commit applies (§7.12).** On a grain with facets beyond the event fold, the handler also works imperatively through the facet accessors — `ctx.kv()`, `ctx.fs()`, `ctx.sql()` — without weakening the split. A *logical* facet stages operations in a per-command overlay: reads see committed-plus-staged, and the staged operations become the command's tagged records, folded only on commit. A *physical* facet mutates a local materialization that is a rebuildable cache (§1), captured into records at local commit and discarded outright on any non-committed outcome (§7.14). The split's guarantee was never "the handler touches nothing"; it is "committed, observable state changes only at the commit point," and that survives intact.
...
The write path chunks file bytes into fixed-size blocks, `put`s each into `ctx.blobs()`, and journals only the resulting `(slice → BlobId)` map; the read path resolves slices to ids and `get`s the blocks back, verified by content (§7.10, **G17**). Deleting or truncating a file drops the slices from the metadata, and the grain's mark-from-roots GC (`ctx.blobs().gc`, §7.10) reclaims the now-unreferenced blocks; `Destroy` drops the whole area. This is what the blob primitive was built for — a grain that is a *durable object with a working set*, not only a small event-sourced value. The filesystem lives **in** the granary crate (rather than a separate crate) so a layered runtime such as the agentic harness depends only on granary for it. (The on-disk, WAL-frame-shipped SQLite store is specified as the SQL facet, §7.14.)

**The filesystem facet.** The filesystem is a **logical facet** (`fs/facet.rs`): `FsOp` records under the fs tag, blocks in blobs, the tree in the composite snapshot — so a grain that needs a filesystem *and* other durable state declares the facet and gets `ctx.fs()` beside `ctx.kv()` and `ctx.sql()`, in one consistency boundary, instead of holding a `GrainRef` to a separate workspace grain (a separate shard map, generally a different leader node, and no cross-grain atomicity). The standalone `Workspace` grain above is the facet's **thinnest consumer** — `State = ()`, `Event = NoEvent`, `Facets = (Fs,)`, every command handler a one-line delegation — kept so a workspace can still be addressed as a grain of its own. Its accessor operations follow the *plan-then-stage* discipline (§7.12): all fallible work (path resolution, block puts) precedes any staging, so a failed operation leaves no trace in the command's batch.

---

### 7.12 The facet model: one substrate, many storage features

**Status: implemented.** The facet seam (`facet.rs`), the KV facet (§7.13), the filesystem facet (§7.11), and the SQL facet (§7.14) are all in the crate, landed in the staging order this section closes with.
...
- A **logical facet** folds. Its records are semantic operations; a pure, deterministic function applies them to an in-memory form (**F1**, the generalization of **G2**), and the host folds only after durability — exactly **G1**. The event fold itself is *facet 0*, the primordial logical facet; the KV map (§7.13) and the filesystem metadata (§7.11) are logical.
- A **physical facet** materializes. Its record is a captured physical delta — the SQL facet's WAL frames (§7.14) — applied byte-deterministically on replay (**F1**), and its live form (the database file) mutates locally *during* the handler. The G1 discipline inverts to **expose only after durability; discard on anything else** (**G20**). The inversion is sound because the materialization is a rebuildable cache (§1) and the §6 gates make un-durable state unobservable: the input gate admits no other command while the append is in flight, the output gate holds the reply, and a non-committed outcome destroys the only copy of the tainted state.

**Staging: the handler writes through the facet; the commit makes it real.** Each declared facet surfaces a `GrainCtx` accessor (§4.3). A logical facet's accessor stages operations in a **per-command overlay**: reads through it see committed-plus-staged, and at append time the host drains the overlay into the command's tagged records — folded into the facet's form only on commit, dropped for free on any other outcome. A physical facet's accessor operates on the materialization directly, inside one implicit transaction per command (§7.14). Both keep §4.2's split intact: committed, observable state changes only at the commit point. Two disciplines follow. *Staging is command-scoped*: a facet write outside a command handler (from `on_activate`/`on_passivate`) has no batch to join and is refused, never silently dropped or deferred. *Plan, then stage*: staged operations commit with the batch even when the handler's reply is an application error, so an accessor operation performs its fallible work (path resolution, blob puts) first and stages only on success — an operation that returns `Err` leaves no trace (the fs facet's operations are built this way, §7.11).

**One composite snapshot.** The snapshot at seq *S* is the tuple of every facet's contribution at *S* — facet 0's encoded `State`, the KV map, the fs tree, the SQL checkpoint manifest — encoded as one `save_snapshot` payload. **G4** applies to the composite as a whole: a composite whose seq exceeds the committed head is ignored entire, and replay runs from `ZERO`. Bulk checkpoint bytes (SQL pages) go to blobs; the composite holds manifests of ids, keeping the snapshot record small.

**Blob liveness is unioned by the host.** Every facet reports its blob **root set** — the fs facet its live blocks, the SQL facet the pages of its live checkpoint manifests (**F3**). The host computes the grain's live set as the union across facets plus the application's own roots, and only the host issues `retain_blobs`; a facet MUST NOT sweep the shared area itself, so one facet's GC can never drop another facet's bytes.

**Compile-time composition.** A grain declares its facets in its type — an associated facet set on `Grain`, defaulting to none — and each accessor is gated by a containment bound, so `ctx.sql()` on a grain without the SQL facet does not compile (the **G10** discipline applied to storage). The declaration statically fixes the grain type's record-tag registry, which is what makes the unknown-tag rule (G19) checkable.
...
- **Durable alarms.** A stored timer (`set_alarm` → `alarm()` handler) that re-activates a grain with no caller present (DO §5). The basis for retries, timeouts, and batch flushes. Durably, an alarm is a logical facet's records (§7.12); the deferred work is the runtime half — the timer and the callerless re-activation — not the storage.
...
- **Third-party facets.** The facet seam (§7.12) stays internal until the built-in set — facet 0, KV (§7.13), fs (§7.11), SQL (§7.14) — has settled the record-tag registry and cross-version compatibility rules; publishing it for out-of-crate facets is a later policy decision. (This item replaces the earlier "alternative record interpretations (SQLite, File)": the SQLite store is now specified as the SQL facet, §7.14, and files as the §7.11 filesystem.)
...
granary/                 # the grain runtime, built on actor-core + actor-cluster
  ...
  fs/                    # the durable workspace filesystem grain (§7.11): metadata folded in
    ...
    facet.rs             #   the Fs facet + the ctx.fs() accessor (§7.11)
    grain.rs             #   the Fs grain: thin command handlers over the facet
