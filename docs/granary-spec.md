...
A grain's in-memory activation is a **cache** of state folded from the journal. The journal, never memory, is the source of truth (§7, invariant **G3**).
...
| **Facet tag** | The stable identifier a multi-facet grain's records carry, dispatching each record to its facet on replay; an unrecognized tag aborts activation (§7.12). Tags are `0..=0x7F`, the high bit being reserved as the record envelope's revision escape (§7.12). |
...
pub trait Grain: Sized + Send + 'static {
    /// The system this grain's activation runs on: a `GranarySystem`, which is a
    /// clustered `ActorSystem` (actor §4) plus the grain host's own seams.
    ...
    /// The grain's declared facet set (§7.12): `()` for none, or a tuple of
    /// built-in facets — e.g. `(Kv,)`, `(Kv, Ws)`. Each declared facet surfaces a
    /// compile-time-gated `GrainCtx` accessor, contributes tagged records to the
    /// same atomic per-command batch (G19), joins the composite snapshot, and adds
    /// its blob roots to the grain's unioned live set.
    ...
    /// The two callerless hooks, both defaulted to a no-op and both decisions
    /// through the §6 barrier (return the events to journal; an empty result
    /// commits nothing, §7.5). Specified where their machinery is: `on_alarm`
    /// with the alarm facet (§7.16), `on_peer_terminated` with death watch (§4.3).
    fn on_alarm(&self, state: &Self::State, ctx: &GrainCtx<Self>)
        -> impl Future<Output = Vec<Self::Event>> + Send { async { Vec::new() } }
    fn on_peer_terminated(&self, state: &Self::State, ctx: &GrainCtx<Self>,
                          peer: &ActorId, reason: TerminationReason)
        -> impl Future<Output = Vec<Self::Event>> + Send { async { Vec::new() } }
}
```
...
4. **The durability barrier lives in exactly one place**, the host (§6), not scattered across handlers.

**Facets stage; the commit applies (§7.12).** On a grain with facets beyond the event fold, the handler also works imperatively through the facet accessors — `ctx.kv()`, `ctx.ws()`, `ctx.sql()`. A *logical* facet stages operations in a per-command overlay: reads see committed-plus-staged, and the staged operations become the command's tagged records, folded only on commit. A *physical* facet mutates a local materialization that is a rebuildable cache (§1), captured into records before the append and discarded outright on any non-committed outcome (§7.11, §7.14). The split's guarantee is that committed, observable state changes only at the commit point.
...
impl<G: Grain> GrainCtx<G> {
    ...
    /// sibling (§7.16). Queued here; the host installs it after the current
    /// handler completes. Watches are activation-local: a rehydrated grain
    /// re-watches from its folded state, and watch-after-death fires
    /// immediately, reconciling what died while it slept.
    fn watch(&self, target: ActorId);
}
```

`GrainCtx` exposes **no** `persist` method and no state mutation (§4.2); state changes only through events folded by `apply`. Under the facet model it additionally exposes one accessor per facet the grain declares — `kv()` (§7.13), `ws()` (§7.11), `sql()` (§7.14) — each gated at compile time by the grain's facet set (§7.12); a facet accessor stages, it never mutates committed state (§4.2).

Beside `on_alarm` (§7.16), the `Grain` trait carries one more callerless hook, defaulted to a no-op: `on_peer_terminated(&self, state, ctx, peer: &ActorId, reason) -> Vec<Event>`, invoked when an actor the grain watched terminates. It runs through the same §6 barrier as any command — one atomic batch, an empty decision commits nothing (§7.5) — and is the basis for liveness-coupled durable state: the machine folds `Detached { FrontDoorLost }` when the front-door member holding an attachment dies (machine §5.1).
...
Granary lifts the Durable Objects gate model (DO §3) onto the framework, so that ordinary sequential handler code is race-free and durable.
...
The host's per-command protocol runs in order:
...
Three ordering rules are normative (invariant **G1**):
...
**Replica placement SHOULD be failure-domain aware.** A shard's *R* replicas chosen by rendezvous alone spread across the cluster but are blind to how the cluster is arranged, so nothing prevents all *R* landing in one rack or availability zone. Since every durability claim here rests on quorum intersection over those *R* replicas (§7.2, §8, **G14**), a concentrated shard turns the loss of one domain from survivable into an unavailable — or, with a minority surviving, an unrecoverable — shard, on a cluster whose other shards look healthy. A deployment that knows its domains SHOULD therefore supply the mapping, and the allocator then takes at most `ceil(R / domains)` replicas per domain, filling from the remaining rendezvous order when a lopsided layout cannot meet the cap (an imperfectly spread shard is strictly better than an under-replicated one). The mapping MUST be a function every node evaluates identically, because the allocation is agreed by every node computing the same rendezvous choice (§7.6); static deployment metadata satisfies this, anything node-local does not. Absent a mapping the choice stays domain-blind, which is the correct default only where the cluster has no distinct domains to spread over.

A shard's leader-election group covers many grains, so the cluster runs O(shards) of them, plus one map group per type (§7.6), bounded by splitting (§7.7), never O(grains), and none is on the data path.

A shard shares leadership and a replica set among its grains, not a serialization point: each grain's records are quorum-appended on their own, so a write-heavy grain never queues behind its shard-mates and no head-of-line block couples grains that merely share a range. Splitting a hot shard (§7.7) rebalances *placement and replication* load across nodes; it is not a knob for write bandwidth.

### 7.2 Ordering is free; durability is a quorum append

Each grain has a single writer, the shard's leader (§8), so ordering is free *per grain*: the host assigns each of a grain's records the next `Seq` with no agreement round, because no other writer proposes records for that grain. The grain's **Replicator** then fans the record out to the shard's replicas and reports it durable once a quorum has stored it (DO §4.3). The only agreement round, electing the shard leader, happens on failover (§8.3), never per write, and never across grains.
...
The records are the grain's `Event`s and state is the `apply` fold (§4.1); a snapshot is the serialized `State`. (Interpreting a grain's records as something other than an event log — the SQL facet's WAL frames — is the facet model's physical class, §7.12/§7.14, over this same Replicator substrate.) The host neither names nor depends on the parts below the seam, which is what keeps the substrate replaceable with no change above it.
...
**Where the store's blocking I/O runs is a seam.** A store call is synchronous by contract — when it returns, its outcome is settled — so for a file-backed store someone must block on the fsync. That MUST NOT be the async worker driving the append: the same worker drives the node's leader-election heartbeats and its other shards' quorum waits, so one slow device stalls heartbeats past the election timeout, shards re-elect, their grains step down (§6), and the next access rehydrates them — a cluster-wide event produced by a local hiccup, and one that generates more I/O while the device is already the bottleneck. A deployment on real storage therefore SHOULD run store calls on a pool of threads separate from the async runtime. This is a seam rather than a fixed behavior because the deterministic simulation (§14) runs the *production* store under virtual time, and a real thread pool there would make a run depend on OS scheduling, breaking the seed-reproducibility of actor §18.1; the default therefore runs the call inline, and a deployment opts into a pool. The seam changes only *which thread* blocks: the guard, the fence promise, and the in-memory apply still happen under the grain's segment lock, in the same order, so it adds no ordering guarantee and removes none.

**The seam is node-scoped, not per grain type.** The pool exists to bound *this node's* concurrent device work, so a node hosting several types MUST share one pool across them rather than giving each its own — as MUST the metrics sink (§13), which an operator reads to judge the node. Both therefore hang off the node's hosting handle rather than off the per-type configuration of Appendix A: a deployment sets them once and hosts every type through it, and a type hosted straight off the system takes the documented defaults (inline I/O, discarded metrics), which is what the simulation requires.
...
**A durable store records which codec wrote it.** A store whose records outlive the process that wrote them MUST carry a `granary.store` stamp ([compatibility](compatibility-spec.md) §3.4) naming the deployment codec, admitted at open before any fence, manifest, or record is read. A grain's *event payloads* are user types under that codec (§4.1, §5) — facet payloads are `postcard` by construction, and a composite snapshot carries its own copy for facet 0 (§7.12) — so a grain with records past its last snapshot, or with no snapshot at all, has nothing else that would notice the codec changing. Unstamped, every such record fails to decode and each failure surfaces on its own as a corrupt-grain activation abort; stamped, the mismatch is one refusal at open naming both codecs, which is what it is: a configuration change. The stamp is per **store directory**, not per record, because every record under one store is written by one deployment's codec and asking per record would spend bytes on the hottest durable path to answer the same question repeatedly. A store that predates its stamp is adopted rather than refused — the codec running at that moment is written down — so the check is not a migration; the limit of that is stated in the compatibility spec.
...
**Linearizable reads are a deferred upgrade (§16), via a check-quorum property on the shard's leader-election group (§8), not a per-read consensus round.** The leader serves reads locally while it holds a **check-quorum lease** (it has heard from a quorum within an election timeout, so no other leader can have been elected), and the activation **self-fences**, returning `Unavailable`/`NotLeader`, when the lease lapses, trading availability for consistency on the minority side (CP, §11). Reads from shard *followers* are a separate deferred extension (§16).
...
**A committed command a node cannot decode MUST stop that node's map, never be skipped.** The map's log is agreed, so an entry one node skips is one its peers apply, and the skipping node's allocation is then not a *prefix* of the committed log but a **gap** in it — a `SplitCommitted` applied without the `SplitStarted` under it. The two nodes disagree about which node owns a range, which is exactly the divergence running the map through consensus was supposed to make impossible, and skipping violates [compatibility](compatibility-spec.md) **V2** directly. So a node that cannot decode a committed command refuses it and stops applying, permanently: its allocation freezes at the last command it could apply, it hands off every shard it leads to a replica whose map is still advancing (§8.3), and its proposer loops stop with it — an allocator sweeping a frozen view would read a shard it never learned about as unallocated and propose a *founding* assignment over the real one, turning a node that stopped reading the log into one that corrupts it. Stopping is safe where skipping was not: a frozen prefix is the state of a node that is merely *behind*, which every path here already tolerates, and a stale leader cannot serve a moved range from it either, because the authoritative barrier is the replica stores' durable append bound (§7.7), not the map. The node stays in the map group and keeps acknowledging its appends — quorum there is about the log being *durable*, not about this build having applied it, and a restart onto a build that can read the entry applies it — and it reports why it stopped, for the health probe that decides it is a node to replace. The payload carries its own boundary, `granary.shardmap` ([compatibility](compatibility-spec.md) §3): a `GRSMAP` magic and a `u16` revision ahead of the JSON, because the `actor.raft.log` stamp it rides covers the **envelope** and not this. So a build meeting a command from a later build refuses it by *revision* rather than by parse failure, which is the difference between an operator reading a mixed-version rollout and reading a damaged log. An **unstamped payload is adopted as revision 1**: every command committed before the boundary existed is still in some cluster's log, and a build that refused them would refuse its own history. Adoption is gated on the magic, so a *stamped* payload at a revision this build does not accept is still refused rather than handed to the older decoder.

The writer emits the stamped form too, rather than taking **V4**'s two releases over it. That ordering exists to protect peers already in the field on a build that cannot read `GRSMAP…` — for which the failure is worse the older the peer is: a build with the refusal above halts, and a build predating it skips the entry and diverges. There is no such fleet here, so the boundary landed in one step; V4 binds the first change made once there is one ([compatibility](compatibility-spec.md) §3).

The map's write rate is low: it changes on shard splits and merges (§7.7), on replica-set reconfiguration, and on node membership changes, never on a grain activation or a grain write. Nodes cache the map and refresh on `NotLeader` (§5.4), so steady-state routing adds no map-group traffic (invariant **G9**). The map group seeds its membership from the actor framework's **leader-based control plane** (actor §9.4.3): granary therefore requires the leader-based control-plane mode; the pure gossip-based AP mode (actor §9.4.4) cannot agree a shard map.
...
A shard that grows too large or too hot **splits**: its key range divides at a boundary, the parent keeping the low half `[start, boundary)` in place and a fresh **child** shard — a new index, and so a new leader-election group (§8.2) — taking the high half `[boundary, end]` on the parent's replica set. Two small, cold adjacent shards **merge** the reverse way: the right shard folds into the left, extending the left's range over it and retiring the right's leader-election group. Split and merge are the elasticity mechanism: the number of shards tracks load and cluster size, so per-shard work stays bounded as the grain count climbs into the millions.

A split MUST be atomic with respect to writes, in this order:

1. **Seal.** The parent's replicas durably record an **append bound** at `boundary` — refuse every append whose name hashes at or above it — on a majority. From that quorum on, no append to the moving range can assemble a write quorum at *any* term, so even a parent leader that has not yet learned of the split cannot land one: a majority of acks would have to include a sealed store. The bound is a store-level quorum barrier, not a leader-local flag, because a leader elected at a higher term but lagging the map log would bypass a flag. An append refused for the moved range surfaces as `NotLeader`/`Unavailable`, never a silent loss (**G5**); the caller re-resolves against the committed map.

2. **Transfer.** For each moved grain, the parent leader recovers its committed head from a read quorum (§8) and copies its committed records, latest snapshot, and blob area to the child's keys on the child's replicas — a majority for the records and snapshot, best-effort for blobs (a down replica heals on access, §7.10). The copy is fence-bypassing under a null term: the child's keys are fresh (disjoint range) and no child leader exists yet, so there is nothing to contest.

3. **Commit.** Only after a full transfer pass does the new mapping commit to the shard map (§7.6): the parent's range shrinks, the child's allocation commits, and the child's replicas create its leader-election group. Only from this commit does any node route the moved range to the child — the map commits *before* either side serves it.

No write is lost or duplicated across the boundary, and no grain is writable in two shards at once (invariant **G15**). Every step is idempotent and derived from committed map state, so a crash or leader change mid-split simply re-drives on the new parent leader. A merge is the mirror image, driven by the **right** shard's leader (which holds the term to fence the right's deposed leaders): seal the whole right range, transfer every right grain into the left's keys, then commit — extending the left's range, dropping the right, and retiring its group (**G7**). Split and merge are one-at-a-time per shard, mutually exclusive with a replica-set migration (which never moves a range).

The size trigger is `shard_target_bytes` (Appendix A): the shard leader auto-splits a shard whose durable footprint grows past it. It is **opt-in** — `0` (the default) leaves the partition to explicit split/merge requests (the admin seam) — because a type whose grains are individually large (a VM image, a big database) wants a higher threshold than a type of many small grains. Splitting on request-rate signals from the §13 event stream is a conforming addition. A split (or merge) commits its progress through the same shard-map log — `SplitStarted`/`SplitCommitted` (`MergeStarted`/`MergeCommitted`) records validated deterministically against committed state, so an invalid or raced proposal is a no-op on every node.

### 7.8 Scaling: where the cost goes

Cost tracks the *active working set* and the *cluster size*, not the total grain count.

- **Consensus groups:** O(shards) leader-election groups plus O(grain types) map groups, bounded by split/merge (§7.7), never O(grains); none carries grain data.
...
- **Activation:** no consensus, no election, no agreement round (§5.2, §10). On the `Quorum` tier it costs one quorum-recovery round-trip to confirm the grain's head (§9); on the `Local` tier it is fully local. Either way it touches no Raft group, which is what makes aggressive hibernation affordable (§10).
...
**Ephemeral.** The sink set is per-activation, never journaled (§1): a cache rebuilt by subscribers re-contacting. On hibernation, migration, or forced step-down it is dropped and `Passivated` emitted (§13); subscribers re-subscribe against the current leader (§5.4) and backfill from their last seq. A subscription does **not** veto idle eviction (`can_passivate`, §10): a hibernated grain produces no records, and the next write re-activates it, at which point subscribers re-establish. (Parking a subscription across hibernation is the deferred *hibernatable connections* extension, §16.)

### 7.10 The grain-native content-addressed blob store (the second primitive)

Records (§7.2–§7.3) and the blobs of this section are granary's **two storage primitives**: the ordered, term-fenced half and the immutable, bulk half of a grain's durable state. They are the only two replication mechanisms in the system — every further storage feature is a **facet**, an interpretation layered over them (§7.12), never a third mechanism.

A grain's journal holds its **small foldable state** — the value `apply` rebuilds on activation (§4.1, §9). Bulk, immutable bytes do not belong there: a workspace's file blocks, a database's pages, any large content a grain references but does not fold. So beside the journal, a grain node owns a second store: an **immutable content-addressed blob area**, replicated to the *same* shard replicas as the journal, reached from a handler or lifecycle hook through `ctx.blobs()` (DO §2.3, §6). A grain stores bulk bytes by content and keeps only their ids in its folded state.

The blob path carries **no `Seq`, no term, and no read-repair**. A content hash names exactly one byte sequence for all time, so there is nothing to order and nothing to agree on: two writers of the same bytes cannot conflict, and a reader proves it got the right bytes by re-hashing them. The single-writer term fence (§8) exists only to keep one *mutable* head from forking, and a blob has no mutable slot. What remains is a single durability question, "is the blob stored on enough replicas yet?", answered exactly as a record's is.
...
- **Reclamation is grain-scoped and root-driven.** The grain knows its own live id set (the ids its state still references), so it reclaims storage with a **mark-from-roots sweep** — `gc(live)` drops every blob not in `live` — and a whole-area `destroy`. The blob store therefore needs **no namespace tombstone, no membership-gated resurrection guard, and no cluster-wide delete fan-out**: deletion targets the grain's own known replicas, and a delete a partitioned replica misses leaves only *orphan bytes* (reclaimed by a later sweep), never a *resurrection of referenced data*, because nothing in the grain's state points at them and only referenced blobs are ever fetched or re-replicated.
- **Lazy hydration and corruption self-heal.** A blob is faulted in on first access: a `get` that misses locally fetches a verifying copy from a peer and backfills the local replica, so a grain that re-activates on a fresh leader and then touches ten files pulls ten files' blocks, not its whole area (DO §5). The same path heals a *corrupt* local copy as well as a *missing* one: a local copy that fails verification is evicted and replaced in place by the verifying peer's bytes. (Eviction-before-replace is required because a content-addressed write of an id already on disk stores nothing.)

**Placement is colocation, not rendezvous.** A grain's blobs ride its *own shard replica set*, not a content-hash-rendezvous spread across the whole cluster: reads are leader-local and recovery comes from the grain's known replicas, at the cost of concentrating a grain's blob bytes on its R replicas. A *cluster-shared* content store — rendezvous-placed, namespace-scoped, for cross-grain dedup, archival, or an external object-store tier — is the complementary **cold** tier of the DO hot/cold split (DO §6), out of scope here (§16). That store is specified separately as the [blob store](blob-store-spec.md), a standalone product beside granary with no in-tree consumer today: this section's blobs, not it, serve the live path. The two are easy to confuse and are not the same primitive — the blob store scopes a blob by a consumer-chosen namespace and places it by content hash across the whole cluster, while a grain's blobs are scoped by `GrainName` and colocated on the grain's own shard replicas.

**Deferred, in lockstep with §7.6.** Today a grain's blobs reach a newly-added shard replica the same way its records do — by **recovery-on-access** (a verified peer-fetch), not by proactive streaming. When §7.6's "stream each grain's committed records and latest snapshot to a new replica before it counts toward quorum" is realized, it MUST stream the grain's blobs too. Range-verified streaming of one large blob against the BLAKE3 tree the id already roots (the Bao encoding) is a further deferred refinement (§16); v1 verifies whole blobs and a consumer chunks beyond a bound.
...
- **Consumption.** The agentic harness declares `Facets = (Ws,)`, hands `ctx.ws().dir_path()` to the sandbox provider at open, and captures in the command that journals each tool outcome, so a tool's result and the files it wrote are one quorum append. The persistent machine (machine §3) declares `Facets = (Disk, Alarm, Ws)`, projects the directory into its guest as a tmpfs `/workspace` — pushed at boot, pulled at each capture quiescent point over vsock — and captures in the same command as its disk manifest, so workspace and rootfs commit in one atomic batch (G19); its file commands write the facet directly while no VM runs, so a hibernated machine's workspace is editable without booting it.

---

### 7.12 The facet model: one substrate, many storage features

**Status: implemented.** The facet seam (`facet.rs`) and every built-in facet — KV (§7.13), workspace (§7.11), SQL (§7.14), disk (§7.15), alarm (§7.16), workflow (§7.17) — are in the crate.

Durable Objects exposes several storage APIs to one object but implements **one** replicated substrate (DO §4.2). Granary applies the same discipline over its two-part substrate (§7.10): ordered records and immutable blobs. A **facet** is a durable storage feature of one grain defined entirely by three answers, and nothing else:
...
**Tagged records, one order, one barrier.** A multi-facet grain's records interleave in the grain's single `Seq` order, each carrying its facet's stable **tag**. All records a command produces — its events plus every facet's operations — append as **one atomic batch** (§6, §7.3), so a command that touches state, the KV map, and the database commits everywhere or nowhere (**G19**). Replay dispatches each record to its facet by tag; a record whose tag the runtime does not recognize MUST abort activation rather than be skipped — a grain's history is never silently misread by a runtime missing one of its facets (**G19**). **Every** record wears the tag, including facet 0's on a grain with no declared facets: one uniform envelope, no second encoding path.

**The tag space reserves its high bit, and is split in two.** A facet tag is `0..=0x7F`; a leading byte at or above `0x80` is not a tag but the escape into a later **envelope revision** ([compatibility](compatibility-spec.md) §3), and a build that does not implement that revision MUST refuse the record by revision rather than report it as an unrecognized tag — the two have different causes and different fixes.

Within the tag range, `0..=0x3F` is reserved for facets defined in this crate and `0x40..=0x7F` for facets defined outside it, so the two can be allocated independently. A tag is permanent — it is the dispatch key for every record ever written under it — so a collision between a built-in facet and a third-party one is not a build error, it is two readings of the same durable byte. Revision 1, the only one defined, is `[tag][payload]` with no escape, so the reservation costs nothing on disk and every record ever written is already a stamped revision-1 record, which lets the envelope change later (a record-level digest, per-record compression, a wider tag space) without migrating a byte. It does **not** version a facet's *payload* schema, which evolves separately: `postcard` appends enum variants safely, so a facet's operation enum grows at its end. Subscriptions (§7.9) are the grain's **event projection**: the typed sink decodes facet-0 records and skips other facets' — their seqs appear as gaps the subscriber's seq-reconciliation already absorbs (G16 holds per projection).
...
**One composite snapshot.** The snapshot at seq *S* is the tuple of every facet's contribution at *S*, encoded as one `save_snapshot` payload: facet 0's encoded `State`, the KV map, the workspace checkpoint manifest, the SQL checkpoint manifest, the disk checkpoint manifest, the alarm deadline, the workflow memo. **G4** applies to the composite as a whole: a composite whose seq exceeds the committed head is ignored entire, and replay runs from `ZERO`. Bulk bytes go to blobs and the composite holds manifests of ids, keeping the snapshot record small — which matters because the record, unlike a blob, is broadcast whole to every replica on each snapshot.

**Facet 0's state is chunked too, by content.** Past a threshold (64 KiB) the encoded `State` is cut into content-addressed chunks and the composite carries only their ids, exactly as the checkpointing facets carry theirs. The cut is **content-defined** rather than at fixed offsets, and the difference is the whole point: a SQLite page and a disk block have fixed addresses, so an unchanged one occupies the same offsets in the next image, but a re-encoded `State` has no such addressing — insert a turn into a folded transcript and every byte after it, plus the length prefixes ahead of it, moves. A rolling hash chooses each boundary from the surrounding bytes, so the chunks after an edit are byte-identical to the ones before it. A chunk already in the grain's root set is **not put again**; that skip is what makes the snapshot cost proportional to what changed rather than to what accumulated, and without it the chunked form would be strictly worse than inline. The chunk puts complete before the record naming them is written, so a durable manifest never references a chunk that is not durable. Under the threshold the state stays inline: the chunked form spends a quorum round trip to save bytes, and at that size the bytes are not worth it.

The payload is **stamped** with a magic and a revision ([compatibility](compatibility-spec.md) §3), read before the body so a composite from another revision is refused rather than misparsed. A snapshot gets a full stamp where a record makes do with a reserved bit, because `postcard` leaves its leading bytes no structure a reader could recognize the format by. The stamped body also records **which codec encoded facet 0's contribution**: that contribution is the one part of a snapshot that is not codec-independent (a user type under the deployment's codec, §4.1, §5, while facet payloads are `postcard`), so without it a changed codec would turn every stored snapshot into a corrupt-grain report rather than the misconfiguration it is. A codec mismatch MUST name both codecs and abort the activation. The state's chunk manifest rides in the body's **extension area** under a *critical* key: it adds a carriage without reinterpreting any byte already defined, so it is an extension rather than a revision, and it is critical because a reader that skipped it would find the inline state empty and rebuild the grain from a default `State`.

**Blob liveness is unioned by the host.** Every facet reports its blob **root set** (**F3**): the workspace facet its checkpoint chunks; the SQL facet the pages of its live checkpoint manifests; the disk facet the blocks of its live checkpoint manifest plus the blocks its post-checkpoint capture records reference; facet 0 the chunks its state was carried in. The host computes the grain's live set as the union across facets plus the application's own roots, and only the host issues `retain_blobs`; a facet MUST NOT sweep the shared area itself, so one facet's GC can never drop another facet's bytes.
...
**Apps are libraries over facets, never facets.** A git-style artifact store is refs in the KV facet plus objects in blobs (§7.10), and a work queue is events or KV. A feature earns facet-hood only when it needs **host participation**: a transaction lifecycle and discard (SQL), a per-command overlay (KV), a timer that re-activates the grain (alarms, §7.16). Everything else layers above, in user code.

**The seam stays internal.** The facet trait is not public API; publishing it for out-of-crate facets is deferred (§16).
...
- **Native, not sugar over SQL.** KV is its own logical facet: no C dependency, its form in memory, unchanged under deterministic simulation (§14), usable by a grain that wants a map without a database file. A grain that needs range queries with predicates wants the SQL facet, not a richer KV.

---

### 7.14 The SQL facet (physical)

**Status: implemented** (`sql.rs`, behind the `sql` cargo feature).

The SQL facet gives a grain a **private, on-disk SQLite database** (DO §2.3, §4.2). The handler runs ordinary SQL through `ctx.sql()` against a local file: reads are leader-local and replication-free (§7.5), writes are transactional, and nondeterministic SQL (`random()`, `datetime('now')`, autoincrement) is fine, because replication is physical, never re-execution. (A *statement-sourced* SQLite grain — SQL text as events, folded by re-execution — remains expressible with plain event sourcing and no facet; it is an application pattern, not this facet.)
...
**Status: implemented** (`disk.rs`). The physical seam it rides is the SQL facet's (§7.14): capture at a quiescent point, checkpoint into content-addressed blobs, discard on non-commit, rehydrate then replay. The disk facet is that machinery with **dirty blocks** for WAL frames and a **raw image** for the database file; capture bytes travel as blobs and capture rides an explicit command. Blob-referencing records use the seam's one post-replay hook, **`rehydrate`**: called once after restore + replay, before the first command, it is where a facet whose records carry blob ids rather than bytes fetches and applies them, so the fold stays synchronous and pure. Every other facet keeps the no-op default.

The disk facet gives a grain a **raw block image**: a fixed-size disk a microVM mounts as its rootfs, the whole-machine analogue of the workspace facet's directory (§7.11). Where the workspace facet captures changed *files* and the SQL facet captures WAL *frames*, the disk facet captures changed **blocks**. A session's dirty set can run to gigabytes, far past the replicator's message bounds, so the block *bytes* never ride the record: a **capture** `put`s each dirty block as a content-addressed blob (§7.10) and stages exactly **one** record, a **capture manifest** carrying the image size and the ordered `(block index, BlobId)` list, which joins the command's atomic batch (§7.12, **G19**). One capture, one record, one commit: a capture is atomic by construction, and a crash can never leave a committed *prefix* of one. The blob `put`s precede staging (*plan, then stage*, §7.12), and blobs orphaned by a failed commit are unreferenced by any root and swept by the host's unioned GC. Replay decodes each manifest into a pending set (the fold, pure — **F1** holds on the recorded bytes); the post-replay `rehydrate` hook then fetches each pending manifest's blocks, verified by content (**G17**), writes each at its offset, and truncates to the recorded size, byte-deterministically, before the first command runs. An unchanged image stages nothing and rides the §7.5 read path.
...
A grain declaring `Facets = (Disk,)` persists the guest's **entire rootfs** over the same two primitives, with no new replication path, where the harness's Native tier persists only the workspace tar (sandbox §3.5). Its worked consumer is the persistent machine ([`machine-spec.md`](machine-spec.md)), which owns *when* the image is captured: a machine's command unit is a whole session, not a single message (machine §4).

- **The scan runs off the async executor.** A capture reads and hashes every block of the image, and an import reads and writes every block of the source; at 1 MiB blocks that is seconds of device and CPU work for an image of any size. It MUST NOT run on the worker driving the node's consensus: a node that stops serving heartbeats for the length of a scan is a node its peers elect around, and the shard they elect around is precisely the one whose machine is being captured — so the checkpoint deposes the activation that owns the guest. This is the §7.4 blocking-I/O seam applied to a facet rather than to the store, and it is what allows a deployment's election timeout to be sized for the network rather than for the longest scan. The scan proceeds a **wave** at a time, so moving it off the executor does not raise the facet's peak memory past the bound the blob-put pipeline already holds, and blocks whose hash still matches the committed index are discarded without leaving the pool.
...
- **The capture seam.** The host MUST obtain exactly the blocks that changed since the last capture, read at a **quiescent point** (the guest paused or stopped, machine §4), so the read sees a stable image, as the SQL capture reads a settled database after the handler returns. The host never captures a *running* guest's concurrently-written image; the consumer quiesces the guest first. Dirty tracking is the **host's own**, not the VMM's: Firecracker's diff-snapshot tracking covers guest *memory* pages and exposes no block-device write bitmap, so the host, which owns the backing file, tracks writes itself. The reference mechanism is **content-hash pruning**: the host chunks the image and diffs block hashes against the last manifest, so the mismatches *are* the dirty set. It is unprivileged and deterministic under simulation; its cost is a full image read per capture. A **copy-on-write overlay per capture epoch**, whose allocated extents are the dirty set, is a conforming alternative and the deferred upgrade (§16). The obligation is the property, not the mechanism.
- **Checkpoints are the snapshot contribution, and they scan and upload nothing.** The committed form keeps a **per-block `BlobId` index** — the last committed capture of each block, which is also the dirty-tracking baseline above — and the contribution is that index serialized as a **full-coverage manifest**: image size, block size, ordered `(block index, BlobId)` list. Every referenced blob was already `put` by the capture (or import) that committed it, so a checkpoint after a capture re-uploads nothing by construction; and the live image is deliberately **not** consulted, because its uncaptured guest writes are not committed state and MUST NOT enter the snapshot (the SQL checkpoint may read its settled database file; the disk's file is never settled while a guest runs). A region no manifest ever covered is absent from the index and restores as zeros — the image file is created sparse and only manifests write it. The live checkpoint manifest's blocks, plus the blocks referenced by capture records after it, are the facet's root set (**F3**); §9 compaction then drops the capture records the checkpoint subsumes.
- **Rehydration.** Composite snapshot → checkpoint manifest → fetch blocks, verified by content (**G17**) → materialize the image → replay decodes capture manifests after the snapshot seq into the pending set → `rehydrate` fetches and applies them the same way (byte-deterministic, **F1**) → boot the microVM against it. The quorum barrier (`head`, §9) is unchanged. Lazy block hydration (faulting blocks in from blobs on first guest access, so a large image activates without materializing whole) is the deferred refinement (§16), the disk analogue of the SQL capture VFS.
- **Bounded, like every physical facet.** The image is fixed-size (a configured maximum); a capture manifest is a small record (indices and ids, never bytes), each block blob is bounded by the fixed block size, and the checkpoint obeys the same blob bounds the SQL and workspace facets obey. A guest cannot make the host materialize an unmetered allocation (sandbox §3.2).

---

### 7.16 The alarm facet (logical)

**Status: implemented** (`alarm.rs`, `alarm_index.rs`).
...
- **Consume-on-fire (G21).** The fired deadline's `Clear` joins the same batch as `on_alarm`'s own records, so the alarm clears atomically with its effects unless the handler re-arms (last write wins in the shared stage): a re-arming handler chains, a consumed alarm stops, and a failed commit leaves the deadline pending to fire again. One arm therefore commits **at most one** effect. The bound is on effects that commit rather than on handler invocations, and the distinction is the whole of it: a fire whose commit fails runs again, which is this invariant holding, not breaking.
- **A pending alarm vetoes hibernation until the index acknowledges it.** A grain owing a wake MUST stay resident — where its in-activation timer fires on time — unless something else will wake it. The per-shard **alarm index** is that something: one small index grain per (grain type, shard) holds a `GrainName → due` map, registered on every alarm change; the alarm **driver** sweeps the index for the shards its node leads and re-activates each grain owing a wake, whose activation re-arms its own timer (**G3**). This covers both deactivation paths, **failover** and **voluntary hibernation**, but the host may hibernate an alarmed grain only when the *current* deadline is **acked-registered**: registration is an `ask` whose reply releases only after the index grain's commit (the output gate, §6), retried on the idle tick until acked, never awaited on the command path. A registration that has not (yet) acked keeps the grain resident. The index is a *hint*, the grain's own facet the source of truth, so a stale entry reconciles on the next activation — a spurious wake at worst, never a lost or double fire.
...
- **The durable core is a memo.** A workflow is a re-entrant decision the grain re-drives after every commit. Each **step** runs an external effect once; the facet journals its result under the workflow tag and folds it into a `step id → bytes` map. On any later drive (a re-activation, a failover) a completed step resolves from the map instead of re-running, so effects are at-most-once across crashes. Step ids are the workflow's stable call-site ordinals, deterministic across re-drives and replay.
  ...
  **The memo is write-once**, and that — not "the effect ran once" — is what a test can assert. The launch guard below is per-activation and never journaled, so a re-activation MAY legitimately re-launch a step whose result has not yet committed, and the same effect MAY therefore run several times; what MUST hold is that the **first committed result wins** and every later `StepDone` for that step commits nothing. Verified by `workflow.rs` (scripted, single-node) and `workflow_swarm.rs` (the same grain under the cluster nemesis), which read a step's memo repeatedly and require every answer to agree.
- **`sleep` is a step whose effect is an alarm** (§7.16); `retry` is a step that re-launches after an alarm-backed backoff.
- **What the grain still wires.** Launching a step's effect off the command path and self-`tell`ing its result back is grain-specific (the effect's type is), so the facet supplies the durable half (the memo's `result`/`record`) plus a generic `StepDone` command that journals a returned result; the "already launched this activation" guard (`LaunchGuard`, generic over the claim key) is ephemeral activation state. A linear `async` DSL over these is deferred (§16).
- **Which half a grain adopts depends on where its results already live.** The memo serves grains whose step results are *not* part of their event vocabulary. A grain whose results are already rich journaled events with their own fold and consumers — the harness agent's `ModelResponse`/`ToolOutcome` records — MUST NOT memoize them a second time under the workflow tag (one decision, two folds); such a grain adopts only the launch-once guard, keyed by its own step identity. The agent is the reference for that posture; a plain effect-runner adopts both halves.
...
**A planned departure SHOULD hand leadership off rather than drop it.** Everything below describes an *unplanned* loss, where waiting out an election is the only option. A node that is being shut down deliberately — a rolling upgrade, a scale-in, a drain — knows it is leaving, and SHOULD therefore hand each shard it leads to one of that shard's other replicas before it stops, using the leadership transfer of actor §9.4.3 item 8. The cost of not doing so is not a correctness problem but an availability one, and it scales with how well the cluster is packed: a node leading *S* shards imposes *S* failover windows at once, each ending in the rehydration of every active grain on that shard (§9), all while the replacement node is starting cold. The handoff is best-effort by nature — it can only target a replica already caught up to the leader's last entry — so a shard whose peers are lagging or unreachable simply falls back to the unplanned path below, and a departing node MUST NOT block its own shutdown waiting for one.
...
- **Snapshot policy.** After a commit, the host MAY persist a snapshot `(head, State)` for the grain via `save_snapshot` (§7.3). The trigger (every *N* events, every *T* of growth) is configuration, not part of the model. Only the shard leader writes snapshots, so a deposed leader cannot (its `save_snapshot` returns `NotLeader`). **One trigger governs every snapshot the host takes**, including the one on idle hibernation (§10): a snapshot is worth taking or it is not, and the answer cannot depend on which path arrived at it.
...
- **Subscriptions are ephemeral activation state.** A grain's record sinks live only in the activation and are dropped on every deactivation path; subscribers re-subscribe against the current leader and backfill from their last seq (§7.9).

Durable **alarms** (§7.16) intersect this lifecycle in two places: a pending alarm vetoes idle hibernation until the per-shard alarm index has acknowledged the deadline, and after a failover that same index re-activates each grain owing a callerless wake, which no message would otherwise touch.
...
**A replica whose storage fails withdraws; it does not crash.** A replica that cannot make a write durable MUST NOT acknowledge it, but it MUST report the refusal rather than terminate: an I/O failure (a full volume, a lost mount, a failing device) makes the store answer a distinguished *failed* outcome to every subsequent record store, recovery read, and blob store, and that outcome MUST NOT count toward any write or read quorum. The effect is that the node drops out of the quorums of every shard it replicates, which is what the majority rules of §7.2 and §8 already handle: writes commit on the remaining replicas, and a failed leader's grains cannot recover on it, so the shard elects around it (§8.3). The refusal MUST be distinguishable from a fence (`Fenced`) and a stale head (`Stale`), which are decisions a healthy replica *made* and which carry a term or head the caller acts on; a failed store has decided nothing and its answer carries no such information. The failure SHOULD be one-way and store-wide, since the fault belongs to the volume rather than to the grain whose write first observed it, and nothing below the seam can establish that the volume recovered. Terminating the process instead would convert a local fault into a cluster event — every shard the node led re-elects and rehydrates elsewhere — which is strictly worse than the degradation the quorum rules were designed to absorb.
...
- `GrainError` MUST be exhaustive at the public API, so callers handle partial failure explicitly (actor §14).
...
The catalogue is machine-readable as `g_catalogue()` beside this crate's conformance suite, guarded by the same drift-test pattern as its siblings (`conformance_catalogue.rs`): a renamed or deleted suite fails the build rather than leaving the column below pointing at nothing. Test files named here are under `crates/granary/tests/`.

| | Invariant | Defined in | Verified by |
|---|---|---|---|
| **G1** | **Single writer per grain.** Only the shard leader appends; the leader-election group elects one leader per term and every append carries the term as a fencing token, so a deposed leader reaches no quorum and a grain's record never forks. A commit advances the grain's head by exactly the batch appended; the host folds only such a contiguous commit and steps down on any other outcome. | §6, §8 | `clustered_grains.rs`, `grains.rs`, `partition_safety.rs` |
| **G2** | **Deterministic fold.** `apply` produces identical state on live commit and on replay from any snapshot/journal prefix. | §4.1 | `clustered_grains.rs`, `grains.rs`, `grain_swarm.rs` |
| **G3** | **GrainJournal is the source of truth.** `head` and state derive only from journal/snapshot returns; in-memory state is never trusted across activation. | §1, §9 | `grains.rs`, `grain_swarm.rs`, `disk_swarm.rs`, `sql_swarm.rs`, `subscription_swarm.rs` |
| **G4** | **Snapshot never shortens the log.** A head is *defined* as the best snapshot's seq plus the contiguous run of records above it (`ReadReply::head` on `Local`, `merge`'s base on `Quorum`), so a snapshot can never outrun the head it is measured against. The host additionally refuses a snapshot the head does not cover, replaying from the journal instead (**G3**). | §9 | `grains.rs`: `a_snapshot_never_outruns_the_head_it_is_measured_against` (the structural identity), `a_snapshot_beyond_the_committed_head_is_refused_in_favour_of_the_journal` (the guard's direction, via a store double) |
| **G5** | **Reply iff durable.** A command's reply is released, and its events folded, only after the entry commits; a `NotLeader`/`Unavailable` outcome yields an error, no fold, no success reply. | §6 | `clustered_grains.rs`, `grains.rs`, `grain_swarm.rs`, `disk_swarm.rs`, `sql_swarm.rs`, `subscription_swarm.rs` |
| **G6** | **Exactly-once activation per node.** The serial gateway activates a name at most once; concurrent requests find the same host. | §5.3 | `grains.rs`, `grain_swarm.rs`, `disk_swarm.rs`, `sql_swarm.rs` (the `ActivationSingletonPerNode` checker) |
| **G7** | **Bounded consensus groups.** The cluster runs `O(shards) + O(grain types)` Raft groups (one **leader-election group** per shard plus one shard-map group per type, §7.6), kept bounded by split/merge, never O(grains) and never one, with **no consensus on the data path**, which is a per-grain quorum append (§7.2). | §7.1, §7.2, §7.6, §7.7 | `clustered_grains.rs`, `shard_split.rs` |
| **G8** | **Activation without consensus.** Activating or hibernating a grain touches no consensus group: no election, no agreement round (on the `Quorum` tier it pays at most one quorum head-recovery round-trip, §9, never a consensus round); only shard membership/leadership/split touches a consensus group. | §5.2, §7.8, §10 | `clustered_grains.rs` |
| **G9** | **Control plane off the data path.** The shard map changes only on cluster events; no grain write or activation contacts it. | §7.6 | `clustered_grains.rs` |
| **G10** | **Type-safe calls.** A command a grain has no `GrainHandler` for does not compile. | §4.3 | `compile_fail.rs` (trybuild) |
| **G11** | **CP under partition.** A shard that cannot reach a quorum pauses its grains' writes (`Unavailable`) and never forks; other shards serve. | §11 | `clustered_grains.rs`, `grains.rs`, `partition_safety.rs` |
| **G12** | **Hibernation round-trip.** A grain evicted when idle re-activates with identical state via snapshot + replay; no acknowledged write is lost. | §9, §10 | `clustered_grains.rs`, `grains.rs`, `grain_swarm.rs`, `facets.rs`, `disk_local.rs`, `sql.rs`, `ws_local.rs` |
| **G13** | **Location transparency.** A call to a local versus remote grain produces observably identical replies and ordering. | §5.4 | `clustered_grains.rs`, `grains.rs`, `grain_swarm.rs`, `blobs.rs`, `blob_swarm.rs`, `disk_swarm.rs`, `sql_swarm.rs`, `subscription_swarm.rs`, `ws_clustered.rs`, `ws_swarm.rs` |
| **G14** | **Lossless failover.** A new shard leader recovers each grain's committed head from a write quorum on activation; by quorum intersection no acknowledged write is lost across a leadership change. | §8, §8.3 | `clustered_grains.rs`, `grain_swarm.rs`, `partition_safety.rs`, `raft_journal.rs`, `ws_clustered.rs`, `ws_swarm.rs` |
| **G15** | **Split/merge safety.** A grain is writable in exactly one shard at any time; a split or merge transfers the committed prefix atomically and loses or duplicates no write. | §7.7 | `grains.rs`, `shard_split.rs` |
| **G16** | **Subscriptions are observational and lossless-by-seq.** Every committed record is delivered to a live sink or left as a gap the sink closes by `load`; subscription delivery never gates a commit, never forks state, and never advances a grain's head. By seq reconciliation a sink reconstructs the exact committed sequence regardless of drops, duplicates, reordering, moves, or hibernation (push ⊆ `load`). | §7.9 | `subscription_faults.rs`, `subscription_swarm.rs`, `grain_swarm.rs` |
| **G17** | **Blob address integrity.** `ctx.blobs().get(id)` returns bytes whose BLAKE3 hash equals `id`, or an error; it never returns wrong or corrupt bytes. Verification is on the read path, after any network transfer; on the `Quorum` tier a corrupt copy falls through to a verifying replica. | §7.10 | `blobs.rs`, `blob_swarm.rs`, `disk_local.rs`, `disk_swarm.rs`, `sql_swarm.rs` |
| **G18** | **Blob durability, idempotence, and grain-scoped reclamation.** A blob `put` is acknowledged only once stored on a write quorum of the grain's replicas (one local copy on `Local`), always including the leader, with no consensus on the path; equal content under a grain stores once; the blob survives the loss of a minority of replicas. Reclamation (`gc`/`destroy`) is grain-scoped, monotonic, and idempotent, and never resurrects a referenced blob. | §7.10 | `blobs.rs`, `blob_swarm.rs` |
| **G19** | **Facet atomicity and interpretation safety.** A command's records across all facets commit as one atomic batch or not at all; a multi-facet grain's records carry facet tags; replay dispatches by tag and aborts activation on an unrecognized tag; the composite snapshot restores every facet to one seq. | §7.12 | `facets.rs`, `disk_swarm.rs`, `sql_swarm.rs`, `ws_local.rs` |
| **G20** | **Physical facets expose only durable state.** A physical facet's materialization is a rebuildable cache: its mutations are unobservable before the commit (the §6 gates) and it is discarded on any non-committed outcome; rehydration from the composite snapshot plus records reproduces it exactly. | §7.12, §7.14 | `sql.rs`, `ws_local.rs` |
| **G21** | **An alarm fires at most once per arm.** One armed deadline commits at most one callerless `on_alarm` effect: the fired deadline's `Clear` joins the same atomic batch as the handler's own records, and the epoch guard makes a superseded timer's late delivery a no-op. The bound is on effects that **commit**, not on handler invocations — a fire whose commit fails leaves the deadline pending and runs again, which is the guarantee holding rather than breaking. | §7.16 | `alarm.rs`, `alarm_cluster.rs`, `alarm_swarm.rs` |
...
- **Hibernatable connections.** Parking a record subscription (§7.9) — or a WebSocket/stream connection — *across* hibernation, holding the registration while the grain sleeps and re-delivering on wake (DO §5), so a follower need not re-subscribe after every idle eviction.
- **Third-party facets.** Publishing the facet seam (§7.12) for out-of-crate facets, once the record-tag registry and cross-version compatibility rules have settled.
...
- **Disk lazy block hydration.** For the disk facet (§7.15): faulting image blocks in from checkpoint blobs on first guest access, so a large rootfs activates without materializing whole. The disk analogue of the SQL capture VFS above.
- **Disk overlay dirty tracking.** For the disk facet (§7.15): the copy-on-write overlay per capture epoch (a device-mapper snapshot or a userspace block device presenting the image), whose allocated extents are the dirty set, replacing the reference content-hash scan for images large enough that a per-capture full read is the dominant cost. Privileged, Linux-only, and isolated behind the facet's private dirty-set seam, so it swaps in without touching the facet contract.
...
let accounts: Granary<Account> = system.granary(GranaryConfig {
    shards: 16,                             // initial partitions of this type's namespace (§7.1)
    replication_factor: 3,                  // replicas per shard (§7.1)
    shard_target_bytes: 256 << 20,          // opt-in auto-split past this size (§7.7); 0 (default) off
    idle_after: Duration::from_secs(300),   // hibernation window (§10); memory is not
                                            //   the scarce resource, round trips are
    snapshot_every: 4096,                   // events (§9); a snapshot reaches every
...
granary/                 # the grain runtime, built on actor-core + actor-cluster
  ...
  disk.rs                # the disk facet (§7.15): a fixed-size raw block image per grain —
                         #   dirty blocks as blobs, one capture-manifest record per capture
  alarm.rs               # the alarm facet (§7.16): one durable deadline per grain, folded
                         #   from Set/Clear records and fired as a callerless AlarmDue
  alarm_index.rs         # the per-shard alarm index (§7.16): the durable GrainName → due
                         #   directory a new leader sweeps to re-activate grains owing a wake
  workflow.rs            # the workflow facet (§7.17): durable step memoization
                         #   (step/sleep/retry) plus the launch-once guard
  facet_blobs.rs         # blob machinery shared by the checkpointing facets (§7.12): the F3
                         #   root-keeping discipline and the chunked blob-area transfers
  testing.rs             # shared simulation invariants over grain events (§14)
```

(`grainref.rs` rather than `ref.rs`, since `ref` is a reserved word.)
