...
- **Consumption.** The agentic harness is the worked consumer: its agent grain declares `Facets = (Ws,)`, hands `ctx.ws().dir_path()` to the sandbox provider at open, and captures in the command that journals each tool outcome — so a tool's result and the files it wrote are one quorum append. The persistent machine is the second (machine §3): it declares `Facets = (Disk, Alarm, Ws)`, projects the directory into its guest as a tmpfs `/workspace` — pushed at boot, pulled at each capture quiescent point over vsock — and captures in the same command as its disk manifest, so workspace and rootfs commit in one atomic batch (G19); its file commands write the facet directly while no VM runs, which is what lets an agent edit a hibernated machine's workspace without booting it. Root-driven blob repair (§7.10, lazy hydration and self-heal) is the consumer's `on_activate` conduct, re-fanning the live checkpoint chunks after membership changes.

---

### 7.12 The facet model: one substrate, many storage features

**Status: implemented.** The facet seam (`facet.rs`) and every built-in facet — KV (§7.13), workspace (§7.11), SQL (§7.14), disk (§7.15), alarm (§7.16), workflow (§7.17) — are in the crate, landed in the staging order this section closes with.
...
Within the tag range, `0..=0x3F` is reserved for facets defined in this crate and `0x40..=0x7F` for facets defined outside it, so the two can be allocated independently. A tag is permanent — it is the dispatch key for every record ever written under it — so a collision between a built-in facet and a third-party one is not a build error, it is two readings of the same durable byte. This split is half of what the deferred decision to publish the facet seam (§16) was waiting on, and it is the half that does not need real use to settle: sixty-four built-in facets is far beyond what this section's "a feature earns facet-hood only when it needs host participation" rule can produce. Revision 1, the only one defined, is `[tag][payload]` with no escape, so the reservation costs nothing on disk and every record ever written is already a stamped revision-1 record. This is what allows the envelope itself to change later (a record-level digest, per-record compression, a wider tag space) without migrating a byte. It does **not** version a facet's *payload* schema, which evolves separately: `postcard` appends enum variants safely, so a facet's operation enum grows at its end. Subscriptions (§7.9) are the grain's **event projection**: the typed sink decodes facet-0 records and skips other facets' — their seqs appear as gaps the subscriber's seq-reconciliation already absorbs (G16 holds per projection).
...
The payload is **stamped** with a magic and a revision ([compatibility](compatibility-spec.md) §3), read before the body so a composite from another revision is refused rather than misparsed. A snapshot gets a full stamp where a record makes do with a reserved bit (§7.12 above), because a snapshot is large and cold: the bytes are noise, and `postcard` leaves its leading bytes no structure a reader could recognize the format by. The stamped body also records **which codec encoded facet 0's contribution**. That contribution is the one part of a snapshot that is not codec-independent — it is a user type under the deployment's codec (§4.1, §5), while facet payloads are deliberately `postcard` — so without it, changing the configured codec would turn every stored snapshot into bytes that fail to decode, reported as a corrupt grain rather than as the misconfiguration it is. A codec mismatch MUST name both codecs and abort the activation, like any other composite that will not restore.
...
- **The capture seam.** The host MUST obtain exactly the blocks that changed since the last capture, read at a **quiescent point** (the guest paused or stopped, machine §4), so the read sees a stable image, as the SQL capture reads a settled database after the handler returns. The host never captures a *running* guest's concurrently-written image; the consumer quiesces the guest first. Dirty tracking is the **host's own**, not the VMM's: Firecracker's diff-snapshot tracking covers guest *memory* pages and exposes no block-device write bitmap, so the host, which owns the backing file, tracks writes itself. The reference mechanism is **content-hash pruning**: the host chunks the image and diffs block hashes against the last manifest (the workspace facet's stat-prune at block granularity), so the mismatches *are* the dirty set. It is unprivileged, deterministic under simulation, and the capture's scan is the facet's only one (the checkpoint below scans nothing); its cost is a full image read per capture, acceptable while the image bound is modest. A conforming alternative is a **copy-on-write overlay per capture epoch** — the guest's block writes land in an overlay whose allocated extents are the dirty set, a capture drains and resets it — which trades the scan away for privileged block plumbing (a raw-only VMM offers no overlay attach, so the realization is a device-mapper snapshot or a userspace block device); it is the deferred upgrade (§16). The obligation is the property, not the mechanism.
...
- **A pending alarm vetoes hibernation until the index acknowledges it.** A grain owing a wake MUST stay resident — where its in-activation timer fires on time — unless the runtime holds evidence something else will wake it. The per-shard **alarm index** is that something: one small index grain per (grain type, shard) holds a `GrainName → due` map, registered on every alarm change; the alarm **driver** sweeps the index for the shards its node leads and re-activates each grain owing a wake, whose activation re-arms its own timer (**G3**). This covers both deactivation paths: after a **failover** (a forced step-down, where staying resident was never an option) and across **voluntary hibernation** — but the host may hibernate an alarmed grain only when the *current* deadline is **acked-registered**: registration is an `ask` whose reply releases only after the index grain's commit (the output gate, §6), retried on the idle tick until acked, never awaited on the command path. A registration that has not (yet) acked keeps the grain resident, the safe default. The index is a *hint*, the grain's own facet the source of truth, so a stale entry reconciles on the next activation — a spurious wake at worst, never a lost or double fire.
...
Durable **alarms** (§7.16) intersect this lifecycle in two places: a pending alarm vetoes idle hibernation until the per-shard alarm index has acknowledged the deadline (staying resident to fire on time is the safe default; an acked registration licenses hibernation, the driver re-activating the grain when it falls due), and after a failover that same index re-activates each grain owing a callerless wake, which no message would otherwise touch.
