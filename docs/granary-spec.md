...
- **What a snapshot costs, and therefore how often.** On the `Quorum` tier `save_snapshot` is a *full-state broadcast*: the encoded composite goes to every replica and is quorum-counted like a record append (§7.3), so the cost is O(state) on the network, multiplied by *R−1*, however little changed since the last one. What it buys is a shorter replay — and replay reads are served from the **local** store (§9 rehydration, §7.5), costing no round trips at all. A deployment SHOULD therefore set the trigger high rather than low, and the counter-pressure is failover rather than steady state: a new leader recovers the records above the last snapshot from a quorum (§8), so a larger gap means a larger one-off recovery read per grain, once per leadership change, against a broadcast every threshold. See [hardware-envelope](hardware-envelope.md) §3.9 for the arithmetic these defaults are derived from. A grain type whose events are large relative to its state inverts the trade and SHOULD lower it.
...
let accounts: Granary<Account> = system.granary(GranaryConfig {
    ...
    snapshot_every: 4096,                   // events (§9); a snapshot is a full-state
                                            //   broadcast, a replay is local reads
