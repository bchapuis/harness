...
This is the read half of **V4**. The writer still emits the unstamped form and flips in a later release, because a peer that has not yet learned the stamp cannot read one — and what it does then is worse the older it is: a build with the refusal above halts, and a build predating it skips the entry and diverges, which is the defect this boundary exists to make legible.
