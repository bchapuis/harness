...
Sections of the granary specification are cited as **grain §N** (its invariants as **G1–G21**); the actor framework ([`distributed-actor-spec.md`](distributed-actor-spec.md)) as **core §N**, the cluster utilities ([`cluster-utilities-spec.md`](cluster-utilities-spec.md)) as **util §N**, and the sandbox specification ([`sandbox-spec.md`](sandbox-spec.md)) as **sandbox §N**. Sections of this document are plain **§N**. Invariants defined here are numbered **H1, H3, …** (there is no H2, §11), kept apart from the granary catalogue (G1–G21, plus the facet contract F1–F4), the core catalogue (core §18.5), and the sandbox catalogue (S1–S5).

> **Design stance.** Granary's idea is that **an object is an actor plus three things**: a name-based virtual identity, a durable event-sourced journal, and a durability barrier on the reply. Everything else (mailboxes, serial execution, location-transparent `ask`/`tell`, membership, supervision) is inherited from the actor model unchanged (grain §1). **An agent is a grain plus three things**: a self-driving loop, a model seam, and a sandbox. Everything else (identity, the journal, the single-writer fence, placement, activation, hibernation, failover) is inherited from the grain unchanged.
>
> What the agent *adds* to the grain is one isolation boundary, placed between **control** and **effect**. Above it, the **loop is the grain's activation behavior**: serial, journaled, touching the world only through the model seam (§4), the sandbox seam (§5.3), and the grain's own journal. A grain is reactive: it waits for a command. An agent is **autonomous**: its activation drives itself forward, model→tools→model, until a run reaches a terminal outcome. Below the boundary, the **effects live in a sandbox**: one isolated environment per session, colocated with the grain's shard leader (grain §5.2), where tools run arbitrarily without endangering the node or another session (§5.1).
>
> An agent **remains an actor**: address a session by name and `ask`/`tell` it, location-transparently (grain §4.3), and define an agent by configuring a *kind*, not by writing a loop. Determinism extends for free: the agent's only new seams (model, sandbox) join the grain's already-virtualized journal, clock, entropy, transport, and spawner (grain §14, core §4.6), so one seed reproduces an entire multi-node agentic run: its model outputs, tool faults, crashes, elections, and resumes.
...
- **A session is a compound AI system, not a model call** (Zaharia et al., [*The Shift from Models to Compound AI Systems*](https://bair.berkeley.edu/blog/2024/02/18/compound-ai-systems/), 2024): the loop is explicit, journaled, and supervised, while the components, model and tools, sit behind seams (§4, §5).
...
The harness is one consumer of the grain, not the only one. The **persistent machine** ([`machine-spec.md`](machine-spec.md)) is a sibling kind over the same substrate: a grain plus a durable disk (grain §7.15) and a network seam (machine §5), reached over SSH rather than driven by a loop, and reusing the sandbox's Firecracker `Native` mechanics (sandbox §3.5). It has no run loop, no model seam, and no sandbox seam. Where the two touch, the machine spec owns the difference; nothing in this document changes.
...
|---|---|
| `SessionId` | `GrainName`: `(grain_type = KindId, key)` (grain §5.1) |
...
The loop reaches the world through two harness seams, **model** (§4) and **sandbox** (§5.3), plus the **grain's journal** it inherits (grain §7), and through nothing else; time, randomness, task spawning, and transport come from the core seams the grain already uses (core §4.6, §7).
...
| **Kind** | A named agent definition (`KindId` → system prompt, toolset, sandbox profile, model parameters, default budget, delegation allowlist). The `KindId` is the session's `grain_type` (grain §5.1); kinds are registered identically on every node (§7.1). |
...
Runs serialize through the session's **turn queue**, a fold projection (§6.2): every accepted turn is journaled at once (`TurnSubmitted`, §7.3) and waits in acceptance order; when no run is live, the loop starts the head by committing **`TurnStarted`** — the one transition that begins a run, the only place the turn's content enters the model-visible transcript, and the append whose commit fires `RunStarted` (§10.4). At most one run is live per session, and because the queue is journal-derived, an acked turn survives crash, hibernation, and migration exactly like the run it waits behind (§7.5).
...
A run MUST end in exactly one terminal outcome, journaled as the run's `RunEnded` record (invariant H3): `Ok(Completion)`, `RunError::BudgetExhausted`, `RunError::Cancelled`, or `RunError::Model(…)` (a model failure no retry policy absorbed, §4.3). All four are journalable: a terminal outcome *is* a committed record, so durability failure cannot be one of them. Durability failure is therefore not a run outcome at all but the grain's outer `GrainError::Unavailable`, which **pauses** the run rather than ending it (§6.4, §7.5). `RunError` is an application error and therefore travels as a value (core §3.2 rule 4; the grain keeps application errors inside `M::Reply`, grain §4.2), not as a transport failure. It is carried in the run's **outcome** (`outcome: Result<Completion, RunError>`), which the caller receives as a `RunCompleted` notification, **not** as the `Submit` reply (§7.3). Transport and durability failure stay separate, in the `GrainError` surfaced on the `Submit` ack (§7.3).

### 3.2 The loop drives itself but never blocks on I/O

The loop is **message-driven**, the discipline the actor and the grain both impose (core §6, grain §6): the activation MUST NOT await the model, a tool, or a durable append inline. It launches each external operation through `Spawner` (core §4.6), and the outcome returns to the activation as an ordinary message; a step is a state the fold tracks, not a stack frame the executor holds. The **I/O lives in the activation's loop, never inside `apply`** (grain §4.1, which MUST be pure and is replayed): the fold stays a deterministic function of the journal, and the loop is the autonomous behavior wrapped around it.

Three consequences, each inherited:
...
Above the boundary, the loop touches the world only through the three seams; it is serial, journaled, and cheap, the mutualization premise of §1 (and grain §7.8, where activation is a local, consensus-free operation). Below it, a tool may do anything *inside* its session's sandbox and nothing outside it. Grain/actor isolation (core §3.5) protects sessions' state from one another; sandbox isolation protects the node, and every other session's effects, from tools.

**Why the boundary cuts here.** Three other placements lose: with **no sandbox**, one session's `rm -rf` or runaway build takes the node (a shard leader hosting many grains, grain §5.2) and every co-located session with it; with the **loop inside the sandbox**, every session pays for an isolated environment to host control flow that is mostly waiting, and the grain's discipline becomes as opaque to the runtime as the tools are; with **a sandbox per tool call**, consecutive calls share nothing, though the checkout, build cache, or running server one step produced is what the next step needs.
...
The tier names are capability sets, not rungs of a number line: the cap that bounds a kind is a *set* of tiers rather than a maximum on a ladder, so a kind MAY hold `Native` without holding `Network` (sandbox §2.2). What each tier grants and withholds, and what a provider MUST guarantee to offer it, live in the sandbox specification (sandbox §2–§3); this document carries only the contract.
...
3. **Launched, never awaited inline** (§3.2): outcomes return as messages, bounded by a per-tool timeout from `Clock`: the call's declared `timeout` (§5.2), or, when it declares none, a configurable harness default (SHOULD default to about **5 minutes**). The timeout bounds the *outcome*, not the effect: the sandbox-side process MAY still be running — and still writing — when the `Timeout` outcome is journaled, so the workspace capture that commits with it (§5.1, grain §7.11) can be **torn**, holding files the straggler had half-written. The journal stays consistent (it records what the directory held at capture), the next call's capture diffs against the last committed index and folds in whatever the straggler finished, and the model, told the timeout, has that transcript in view when it reacts.
...
5. **Technology-opaque, within a tier.** Whether a tier's environment is a process, a container, or a microVM is the provider's secret; the simulator's scripted sandbox is one more implementation of the same trait (§12). *Which capabilities a call holds* is declared per tool (§5.2), journaled per acquisition (§5.6), and agreed cluster-wide (§7.1), never improvised by the provider. The seam still sees one `Sandbox`: tiers are provisioned within it, never as separate seam objects.

### 5.4 Tool failure is a transcript value

A failing tool does not fail the run. The harness journals its `ToolError` (a timeout, a sandbox-side crash, a failed delegation (§8.2), an unknown tool name, or malformed arguments) as that call's outcome and **returns it to the model** as the tool result, for the model to react to. The only abnormal run endings are the three `RunError` outcomes of §3.1, and "a tool misbehaved" is not one of them. Unknown and malformed calls in particular are synthesized outcomes, not protocol failures; the registry-as-allowlist already guarantees nothing was executed. Malformed is structural, not schematic: the loop rejects a call whose input is not a JSON object (and a `delegate` input that does not deserialize, §8.1) before dispatch, and does not validate arguments against the declared `input_schema` — the schema is interface metadata for the model (§4.1) and digest-covered agreement (§7.1), and validation beyond shape belongs to the executing tool, whose rejection surfaces as its own `ToolError` outcome like any other failure.
...
A session's transcript is the grain's journal: each **record** is a grain `Event` (grain §3), appended in `Seq` order, totally ordered per session, durable on a shard quorum. The record types are the harness's event vocabulary: `SessionCreated`, `TurnSubmitted` (a turn accepted into the session's queue, §7.3), `TurnStarted` (the queue's head left the queue and its run began, §3.1), `ModelResponse`, `TierAcquired`, `ToolOutcome` (carrying a `CallId`), `ChildRun`, `WorkspaceReset`, `RunEnded`, and `CancelDelivered` (a propagated cancel's delivery receipt, §9.2). `apply` (grain §4.1) folds them into session state. `apply` MUST be pure and deterministic (grain G2): it runs on the live commit path and on every replay, and the agent's only non-determinism (model output, tool effects) is captured *as committed records*, never re-derived in the fold.
...
- a **turn's acceptance** (`TurnSubmitted`) before the `Submit` ack releases (§7.3), and the **run start** (`TurnStarted`) before the run's first model call issues (§3.1);
...
Each node's harness is configured with the same `KindId → Kind` map: system prompt, `ToolRegistry`, `SandboxProfile`, model parameters, default budget, delegation allowlist (§8.1). A `KindId` is a `grain_type` (grain §5.1); the kind is the grain type's configuration (its `GranaryConfig`: shard count, replication factor, idle window, snapshot policy; grain Appendix A). Kinds are code-and-config, agreed cluster-wide like the codec (core §5): a session created with a kind MUST be resumable on any node, so every node MUST register every kind. A `SessionCreated` record pins the session's kind and a **digest** of its definition (§10.5); the digest covers, among the definition's fields, each tool's declared tier and the profile's tier cap (§5.6). Activation on a node missing that kind is a deployment error that fails the triggering call with `GrainError::Call(System)` before any run starts (nothing journaled, no `RunStarted`), never a silent fallback.

Registration is knowledge; a node's **directory** — the kinds it *hosts* (grain types it serves, seams injected) plus the kinds it only *routes to* (the cluster-client path, grain §5.3) — is per-node deployment shape. Harness construction MUST reject a directory that does not close over delegation: every hosted kind's delegation allowlist (§8.1) must be contained in the directory, because the parent's own node launches the child `Submit` (§8.1) and delivers owed cancels (§9.2) through its route for the child's kind — uncovered, every delegation to that kind would fail per-call, and an owed cancel would wait on a better-configured node leading the shard (H5's liveness). The rejection is at build, as loud as a duplicate tool name (§5.2), so the gap is a deployment error, never a mid-run discovery.
...
**Hibernation (idle stop).** After a configurable idle interval the leader MAY hibernate a session (grain §10): run nothing that mutates state, snapshot to bound the next replay, release the sandbox (H8), and drop the activation. It MUST NOT hibernate a session whose run is live or whose queue holds an accepted turn (§3.1) — both mean work is owed now. Hibernation needs no flush (anything worth keeping is already a record, §6.2), and it releases the sandbox — the environment's processes and containers; the workspace's durable subtree survives in the journal and rematerializes on reactivation (grain §7.11, §5.5). `idle_after` SHOULD default to **minutes**, inheriting the grain default (grain §10) rather than the Durable Objects eviction window it is modelled on: reactivation is cheap but not free (a quorum head-recovery plus a snapshot-bounded replay), and the memory eviction reclaims is not the resource under pressure here. The idle timeout trades sandbox cost, the expensive resource mutualization shares, against environment continuity (warm containers, regenerable build caches).
...
Reading a run is deliberately **not** a session command: `tail` (§10.2) rides granary's gateway-level event read (grain §7.5), a framework built-in like `subscribe`, so it appears in no allowlist here and never touches the activation.

†`Submit` replies with an **ack, not the run's outcome**: `Accepted { turn: TurnId, status: Started | Attached | Ended }`, released by the output gate the moment the `TurnSubmitted` record commits, an ordinary output-gated grain reply (grain §6). Acceptance and start are distinct journal facts: `TurnSubmitted` enqueues the turn (behind the live run, if any) and releases the ack — so **an acked turn is durable**, surviving crash and migration in the fold's queue (§3.1) rather than in any activation's memory — and the loop's dispatcher later commits `TurnStarted` when the turn reaches the head. `Started` therefore means "newly accepted", whether the run begins immediately or behind the queue; there is deliberately no distinct queued status, since every accepted turn is queued for at least the instant before its `TurnStarted`. Its `Err` arm, `SubmitReject`, is an *application* failure carried as a value inside the reply, kept distinct from the transport `GrainError` outside it (core §3.2, §14): a caller-contract violation is permanent — the same submit always rejects the same way — where a transport failure a peer move could clear. The two cases are `KindMismatch` and `ContentConflict` (§7.4). Each command is delivered through `GrainRef::ask` (grain §4.3), so the call surfaces `GrainError` (transport — including an unregistered manifest — `NotLeader`, `Unavailable`, grain §12) outside the reply. `Cancel` likewise commits one `RunEnded { Cancelled }` record and acks — ending the live run, or ending a still-queued turn before any run starts (§9.2).
...
**Why the outcome is a notification, not the `Submit` reply.** A reply to an `ask` is an actor `ReplyHandle` (core §4.5): synchronous, resolved exactly once *within the delivery of that one message*, and **not storable across later messages**. A run spans many inbound messages (each model and tool outcome, §3.2) between `Submit` and `RunEnded`, so the run's `Completion` *cannot* be the `Submit` reply: there is no handle to hold across that gap. So `Submit` acks the moment its `TurnSubmitted` commits, and the outcome is delivered later to a reply-to `ActorRef` the caller placed in the command (core §3.3: "An `ActorRef` MAY be a field of a message"). Completion arrives as one more message into the loop, exactly like every tool outcome, and "attaching to a live run" (§7.4) is registering another reply-to.

The `Submit` `ask` returns promptly (it commits one record), and nothing a caller holds bounds the run itself: budgets (§9) bound its tokens and steps, not its wall time (tool timeouts, model retries, and child runs compound), so a run MAY outlive any caller. A caller that gives up, times out its wait, or crashes leaves the run untouched; it recovers the outcome by re-submitting the same `TurnId` to re-register (§7.4) or by following `tail` (§10.2). There is deliberately no status message: a status view (head `Seq`, live run if any, spend so far) is a client-side fold over `tail`'s records, not a second reply type.
...
A read (`tail`, and any future query that emits no record) commits nothing (grain §6 step 2, §7.5) and is a local, replication-free read of the shard leader's journal (§10.2). Its consistency is **read-your-leader**, not linearizable under partition (grain §7.5): an isolated minority leader, deposed but not yet fenced, MAY serve a slightly stale `tail` until it steps down. Writes never fork; only these reads can be momentarily stale, and only on the minority side. A linearizable read is a deferred grain upgrade (grain §7.5, §16), not a harness mechanism.
...
let s = h.session("researcher", SESSION_ID)?;      // a GrainRef<Agent>; name→shard is a pure local hash,
                                                   //   no I/O (grain §5.1, §5.4); the one failure is a
                                                   //   directory miss — a kind this node neither hosts nor
                                                   //   routes. Kind names arrive from outside (a URL path
                                                   //   at a gateway), so the miss is a value, not a panic
...
`SessionRef` is `GrainRef<Agent>` (grain §4.3) with a thin agent-facing surface. `prompt(turn)` is a **blocking convenience composed at the edge, not a protocol primitive**: it spins an ephemeral local actor as the reply-to, `Submit`s the turn carrying that ref, awaits the one `RunCompleted` message, and presents `Result<Result<Completion, RunError>, GrainError>`. If its own wait lapses, it re-`Submit`s the same `TurnId` to re-register and keeps waiting. A caller that prefers not to block uses `Submit` with its own reply-to (a parent agent does exactly this, §8.1) or observes through `tail`. The ephemeral reply-to's lifetime is **its caller's wait, not the delivery**: an attempt that ends without an outcome — a lapse, a rejection, a transport failure, the caller abandoning the wait — MUST stop its mailbox, so a re-attach *replaces* the mailbox rather than accumulating one per attempt and no path leaks the actor; a notification racing the discard is an ordinary lost delivery, recovered by re-contact (§7.3).
...
- a `Submit` whose `TurnId` is already journaled MUST NOT start a second run (one `TurnSubmitted` record, G1): if the turn is still queued or its run live, the activation **registers the new reply-to** on it (`status: Attached`), to notify at `RunEnded`; if the turn already ended, the activation immediately **`tell`s the recorded outcome** (`status: Ended`), read from the `RunEnded` record. The dedup, and the recorded outcome, are a fold over the session's own records — queued turns included, so no ephemeral second check exists — and both are exact even across a resume;
...
- the `reply_to` is **transient routing, not part of the turn**: it is not journaled and is **excluded from the content-equality check**, so re-submitting the same `TurnId` with a fresh reply-to is the ordinary re-attach, not a mismatch. The check covers the turn's content **and its literal `budget` field** — a re-submission that changes the budget is not a silent attach, and `None` is not `Some(default)`, so whether two submissions conflict never depends on the kind's *current* default. (The comparison digest is fold-local, recomputed from the journaled `TurnSubmitted`, which records the resolved budget plus whether it was explicit; the encoding is length-prefixed, so content and budget cannot collide by juxtaposition. The digest is a fast 64-bit content hash — a guard against *accidental* reuse of a `TurnId` with different content, not a cryptographic commitment: a deliberately crafted collision would attach rather than conflict, so the check is a caller-bug detector, never a security boundary — and no isolation rests on it, since a caller who can address the session at all already holds it; a gateway scopes session names by principal for exactly that reason.) A re-submission whose turn differs under that check (`SubmitReject::ContentConflict`), or whose `kind` differs from the grain's own type (`SubmitReject::KindMismatch`), is a caller bug, rejected as a value in the reply and journaling nothing — never as a transport `GrainError`, because a caller must not retry it (§7.3†). Delegation re-execution stays safe by construction: a parent re-submits the budget read from its journaled `ChildRun`, re-deriving the same literal (§8.1);

The **subscriber set** (the reply-tos registered for a live run) is ephemeral activation state: like the held tiers and the sandbox (§5.5, §5.6), it is rebuilt by callers re-contacting after a resume, never journaled.
...
Resumption is **caller-driven**: the grain does not scan for orphaned work (grain has no cross-grain enumeration), so a run interrupted by a crash resumes only when a message next reaches the session: the re-submitted `TurnId` of a caller that wants its answer, a parent's re-executed delegation (§8.1), a `Cancel` (idempotent on an ended run, and the explicit nudge for any cancel propagation the journal still owes, §9.2). The same contact resumes the **queue** (§3.1): an acked turn a crash left waiting is fold state, and the first message to reach the resumed session drives the dispatcher, which starts it once no run is live. A run nobody asks after stays interrupted, and its budget bounds what it can ever cost (§9.1). H3's liveness is conditional on exactly this contact — and so is H5's for a session whose owed propagation a crash interrupted.
...
1. the parent journals the delegation (a `ChildRun` record: the child `SessionId` and the child turn's `TurnId`, both derived deterministically from the parent session, the parent's `TurnId`, and the delegation's `CallId` (§5.2), plus the carved budget, §9.1; one run may delegate many times, so the call, not the turn, is the unit of derivation); the derivation length-prefixes each component, so distinct `(session, turn, call)` triples always derive distinct ids — ids are application-chosen, and a bare separator join would let crafted ids collide; a re-executed delegation re-derives the same pair (§5.5), and cancel propagation reads it from the record (§9.2);
2. the launched task `Submit`s to the child session (a `GrainRef`) carrying a **reply-to** ref and **awaits the child's `RunCompleted`** notification (§7.3); it never block-`ask`s the child, whose run may be arbitrarily long. The await re-attaches on the submit-deadline cadence (each lapse re-registers on whatever activation then hosts the child, §7.5; the derived `TurnId` keeps every re-attempt safe, H7) and is bounded in total by a **budget-scaled wait**: a configurable floor plus a configurable per-step allowance × the child's carved `steps` (the floor SHOULD default to about **10 minutes**, the allowance to about the per-tool timeout, §5.3 item 3) — the carve is the right scale because the step bound is compositional (§9.1 item 3), so it bounds the whole subtree's model calls, not one level's. Past the bound the delegation resolves as a synthesized `ToolError` for the parent's model to react to (§5.4); the lapse cancels nothing — the child runs on, its budget the backstop (§9.2 item 3), and its `ChildRun` now has a journaled outcome, so a later cancel of the parent no longer propagates to it (§9.2 item 2). A *consecutively unreachable* child is bounded separately, by a transport retry limit with backoff (any accepted submit resets it), surfacing the same way (§5.4). The child is a **full session**: a grain in its own right, journaled, budgeted, placed on its own shard wherever the cluster puts it (grain §5), supervised on its leader;
...
4. **What the bound is.** Spend is *journaled* usage. A model call whose response never commits (issued by a leader deposed mid-step (§6.2), whose record loses to the contiguity guard, grain §6) is discarded uncounted, though the provider bills it: the budget bounds the session's recorded spend, and the provider's invoice exceeds it by exactly the speculative work a leadership change cost.

### 9.2 Cancellation

1. `Cancel` is a message (§7.3); because handlers never block on I/O (§3.2) and the input gate admits it between appends (grain §6), it takes effect at the next message boundary, not after the current model call returns. It names the run it cancels (`turn`): a `Cancel` naming an ended or unknown run is an idempotent no-op, so one delayed in flight never kills the named run's successor. One naming a **still-queued** turn (§3.1) journals that turn's `RunEnded { Cancelled }` directly: the fold drops the queue entry, subscribers are notified from the recorded outcome, and — no run ever began — no `RunStarted`/`RunEnded` event pair fires (§10.4).
2. On cancel, the session journals `RunEnded { Cancelled }`, notifies every registered reply-to with `RunCompleted { outcome: Err(RunError::Cancelled) }` (§7.3), discards subsequent outcome messages of that run (§3.2), and **owes propagation**: folding the terminal record moves the run's unresolved delegations (each `ChildRun` without a journaled outcome, §8.1) into the fold's *owed-cancels* fact, and the post-commit `Advance` sends `Cancel { turn }` to each owed child, each of which cancels its own subtree. The owed set is a projection of the journal, never activation state, so a crash between the terminal commit and the send loses nothing: the next activation re-derives the debt and re-sends when a message next drives the session (§7.5; a re-sent `Cancel` nudges owed propagation explicitly).
3. Delivery is journaled. A child's ack releases only after the child's own commit (its `RunEnded { Cancelled }`, or nothing for an already-ended run — the grain's output gate), and the parent then journals **`CancelDelivered { turn, call }`**, which clears that child's owed fact; until it commits, a re-send is safe because a cancel of an ended or unknown run is a no-op (item 1). Transport gives each send at-most-once (core §7.2, grain §2.2), so the activation re-attempts an unacked send a bounded number of times and then abandons it *for itself only*: the debt stays in the fold for the next activation. The guarantee is two-layered: once faults cease and a message reaches the session, propagation completes within bounded logical time (H5); under unhealed faults, the child's **budget is the backstop**, since every run is bounded with or without the cancel arriving (§9.1).
...
Operating agents means seeing what the model said, which tools ran, what a run cost, and how a delegation tree unfolded. The design already centralizes the one record that carries all four: the grain's journal.
...
`SessionRef::tail(from: Seq, limit) → Result<EventHistory<Record>, GrainError>` (§7.4; `EventHistory` is granary's event-read reply, grain §4.3) returns at most `limit` committed records after `from`, plus the journal's compaction `base`: an idempotent, replication-free **read of the leader's journal** (grain §7.3 `load`), riding granary's gateway-level event read (grain §7.5) rather than any session command (§7.3). The gateway serves it **without get-or-activating the session**, so a poll against a hibernated session leaves it asleep — observation costs a journal read, never a rehydration or a workspace rematerialization — and the activation keeps no mirror of the raw records: the fold holds projections only (§6.2), so session memory and snapshots do not grow a second copy of the transcript. Four consequences of reading the journal itself:
...
- **`limit` is bounded.** `SessionRef::tail` clamps `limit` to a page cap (SHOULD default to about **1024** records), and the gateway independently bounds each wire page server-side (grain §7.5), so no single read materializes an unbounded reply; a short page — fewer records than asked — occurs only at the head, so an empty page means no record follows `from` *that the journal still holds* (truncation is announced by the base below, never inferred from page shape).
- **History compacts, and the read says so.** Snapshot compaction (grain §9) drops the records a session snapshot subsumes, so a long transcript's oldest records eventually exist in no journal (folded state is untouched — the snapshot subsumes them; only *history* truncates). Every reply therefore carries the compaction `base` (grain §7.5): `base <= from` means the page is complete history from `from`; `base > from` means the records in `(from, base]` were compacted away and no re-read recovers them. The follower (`SessionRef::follow`) surfaces the same fact as an explicit `Truncated { base }` step — the caller is told *before* the cursor resumes past the gap — and the HTTP surface mirrors both (a `base` field beside the records page; a `truncated` stream event whose event id is the base, so a `Last-Event-ID` reconnect resumes after it). Truncation MUST be surfaced, never papered over: a reader from `Seq::ZERO` handed a silently short page would mistake a snapshot boundary for the whole transcript, and the journal's slot gaps alone cannot disambiguate — legal facet gaps look identical (first bullet).
- **The read can fail.** A journal read that fails (I/O, corruption) surfaces as the grain's `Unavailable` (grain §12): transient, safe to re-ask — a read commits nothing — and never masked as an empty page.
...
A `tail` MUST NOT trigger a write or change any activation lifecycle: served at the gateway, it neither wakes a hibernated session nor even refreshes an active one's idle clock, so continuous observation and hibernation's economics (§7.2) no longer trade against each other. Push-based observation is future work (§13); `tail` is deliberately the smallest interface that makes a run watchable.

### 10.3 Tree correlation

Every `SessionCreated` records its session's **root**: its own `SessionId` for a session created with no parent, the parent's `root` for a session created by delegation (§8). The field is the transitive closure of the parent links, denormalized so any record, event, or log line can name its logical request in O(1). Together with the recorded parent links, `root` stitches one delegation tree across shards, nodes, journals, and logs: the trace propagation core §16 recommends, with no identifier minted for it. Mapping trees onto external trace systems is future work (§13).
...
The sandbox pair is *not* derivable from records, because the sandbox is not session state (§5.5). Run boundaries (`RunStarted`/`RunEnded`) and per-call `usage` (`ModelCompleted`) are the minimum needed to check H3/H4/H7 on the stream, and `ToolCompleted` carries the node a sandboxed call resolved on, which lets the H8 checker place each call inside a `SandboxBound`/`SandboxReleased` window; everything else about a session's lifecycle is read from the grain's events.
...
The harness catalogue mirrors the granary, core, and utilities catalogues and is machine-readable alongside the harness's conformance suite (`harness_catalogue()`), guarded by the same drift-test pattern. It lists only what is specific to the *agent*; the grain's own guarantees are cited in the grain-basis column rather than re-proved here. The deterministic fold (G2/G3) and single activation (G6) reappear as the agent-level H1 and H6, which add resume-equivalence and autonomy to them, while the single-writer fence (G1) has **no** harness invariant at all: it is wholly the grain's, appearing only as the basis other agent invariants build on (H6's one leader, H7's single commit). There is deliberately no **H2**: the write-ahead obligation that number would carry is wholly the grain's output gate (§6.3, G5), and H3–H8 keep stable numbers.

| # | Invariant | Defined in | Verified by | Grain basis |
|---|---|---|---|---|
| H1 | **Deterministic fold and resume.** Session state is a pure fold of the journal; a session resumed from any committed prefix behaves byte-identically to one that never stopped, given the same subsequent model and tool outcomes. | §6.2, §7.5 | differential resume-vs-uninterrupted test (`conformance_resume.rs`); seed-reproducibility sweep (`harness_swarm.rs`: `the_event_stream_reproduces_byte_for_byte_per_seed`) | G2, G3 |
| H3 | **Run termination.** Every `RunStarted` is followed by exactly one `RunEnded`; once faults cease, partitions heal, and a message reaches the session, no run remains pending past its budget's bound. The `RunEnded` record is the single source of truth; the `RunCompleted` notification is best-effort (`tell`), and a lost one is recovered by re-contact (§7.4), never retried. | §3.1, §7.5, §9 | continuous checker (`harness-run-discipline`, pairing); scenario and swarm sweeps (`conformance_run.rs`, `cluster.rs`) for bounded completion under caller re-submission | G14 (lossless failover) |
| H4 | **Budget bound.** A run issues no model call after exhaustion; output spend never exceeds the remaining budget at call time; own spend plus children's carve-outs never exceeds the budget, at every level of a delegation tree. | §9.1 | continuous checker (`harness-run-discipline`) over `ModelCompleted`, scoped to journaled spend (§10.4); journal audit and tree scenario (`conformance_budget.rs`) | (none) |
| H5 | **Cancellation.** After a cancel is journaled, the run and, once faults cease, every descendant run end `Cancelled` within bounded logical time, issuing no further model calls. | §9.2 | scenario and swarm tests (`conformance_cancel.rs`); crash-window scenario (`cluster.rs`: leader crash between the terminal commit and the propagating send); checker (`harness-run-discipline`) for post-cancel `ModelCompleted` | (none) |
| H6 | **Single autonomous activation.** A node never runs two concurrent activations of one session; on a converged cluster, at most one activation per session is live; an addressed session activates within bounded logical time of a message reaching its shard leader. | §7.2, §7.5 | continuous checker (`harness-event-grammar`, the per-node half: no model completion outside an activation), inherited grain checker (G6), and the converged and liveness halves of the cluster scenario (`cluster.rs`) | G6, grain §8 |
| H7 | **Idempotent submission.** A re-submitted `TurnId` never starts a second run: it registers the caller's reply-to for notification, or `tell`s the recorded outcome if the run already ended, under any injected duplication or caller retry. | §7.4 | continuous checker (`harness-run-discipline`, one `RunStarted` per `(session, turn)`); retry scenarios (`conformance_run.rs`) | G1 (one commit) |
| H8 | **Effect containment.** An activation binds at most one live sandbox and releases it on deactivation; every sandboxed tool call executes in the sandbox bound to its issuing activation, never another session's; sandbox loss surfaces in the journal (`ToolError` outcomes, a `WorkspaceReset` record, §5.5), never as silent corruption. | §5.3, §5.5 | continuous checker (`harness-event-grammar`: `SandboxBound`/`SandboxReleased` within the grain's activation window, calls within the bind window); crash/loss scenarios (`conformance_sandbox.rs`); cross-session isolation by construction | grain §5.2 (colocation) |
...
- **Model:** latency, `RateLimited`/`Overloaded` bursts, `ContextOverflow`, unknown tool names, malformed or schema-violating arguments (rejected by the loop and the executing tool respectively, §5.4), pathological tool-call loops (exercising budgets).
...
let s = h.session("researcher", SessionId::new("report-42"))?;  // GrainRef<Agent>; Err ⇔ the kind
                                                                //   is not in this node's directory (§7.4)
