...
- **`limit` is bounded.** `SessionRef::tail` clamps `limit` to a page cap (SHOULD default to about **1024** records), and the gateway independently bounds each wire page server-side (grain §7.5), so no single read materializes an unbounded reply; a short page — fewer records than asked — occurs only at the head, so an empty page means no record follows `from`.
