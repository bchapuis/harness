...
**Status:** open — nothing built, and unlike the rest of §1 the defect is live in the tree today rather than a gap in what a future change could do safely.
...
Believing (1) needs mixed-version simulation rather than a fixture: a workload running one node on a build that knows a command variant and its peer on one that does not, asserting the cluster refuses rather than diverges. The harness for that exists — §1.1's `Rollout` sweeps already drive a cluster across releases — so the missing pieces are the workload and a second definition of the command to vary, not the machinery. Done when a node that cannot apply a committed shard-map command halts, and a test proves it halts rather than drifts.
