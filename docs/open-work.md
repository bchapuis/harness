...
- **Granary alarms have no continuous sweep, and the attempt to write one found a defect first.** `alarm-cluster/leader-crash` sweeps 24 seeds, but no `ClusterWorkload` runs alarms under the continuous nemesis with at-most-once firing as a checker. The blocker is not the workload. **An alarm-wired granary cannot commit under frame loss**: with `granary_with_alarms` and a 15% drop / 10% duplicate policy — no partition, no crash — a grain's first command activates, never commits, and is passivated, while the caller hangs until its own deadline rather than being told. The identical workload built with plain `granary` commits under the same loss, on the same seed, with the same number of Raft groups (both arms host the index granary; only the timer's wiring differs). Sixty seconds of call budget and ninety of virtual time do not change it, so it is stuck rather than slow. Two things to separate: why the append fails at all, and why its failure reaches the caller as a timeout instead of the `Unavailable` the unwired path returns — a command that cannot commit should say so. Unproven suspects, both unbounded `ask`s that nothing ages out: the driver loop's per-sweep index read (`grainref.rs:1085`, every 500 ms per led shard per node) and the per-commit index registration (`host.rs:835`).
