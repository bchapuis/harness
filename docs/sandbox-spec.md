...
The first provider is `harness-sandbox`, realizing §3.5's choices for the tiers it offers: `Workspace` behind a cap-std handle (the default feature), `Compute` behind hermetic wasmtime guests (feature `compute`), and `Native` behind an OCI container driven through the docker CLI (feature `native`, §3.5's development fallback) or behind a Firecracker microVM (feature `firecracker`, §3.5's reference choice — one VM per activation, the workspace synced over vsock, Linux + `/dev/kvm` at runtime); `Network` remains unoffered there, failing as a `ToolError` outcome (harness §5.4), and the standalone deployment's `LocalSandboxes` remains the degenerate unconfined `Native`-only provider of §5. The catalogue is machine-readable as `s_catalogue()` beside that crate's conformance suite, guarded by the same drift-test pattern as its siblings; the entries still marked *(future)* name the intended method in the established vocabulary (core §17's verification kinds), not an existing test.
