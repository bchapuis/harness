...
- **Fine-grained liveness.** Within a live namespace the store cannot know which blobs are referenced, so it never reclaims an *individual* blob on its own. The **namespace** is the unit of liveness instead: a consumer groups blobs that share a lifecycle (a tenant, a workspace, a snapshot-set) under one namespace and reclaims them together with `delete_namespace` (§5.3). The store needs no root set and no garbage collection.
...
- A **`BlobId`** is the 32-byte BLAKE3 digest of the blob's bytes. It is `Copy + Eq + Hash + Ord + Serialize + DeserializeOwned`, rendered in lowercase hex for display. BLAKE3: every `get` re-hashes its bytes to verify them (§4, **B1**), so hashing throughput sits on the read path, and the digest is itself a Merkle root, which the range-verified streaming deferred to §10 builds on.
...
- **No drop on rebalance.** A node that is no longer an owner of a blob it holds keeps the extra copy; reconcile tolerates over-replication and never risks under-replication. Such a now-misplaced copy is reclaimed only when its whole namespace is deleted (§5.3).
...
The catalogue is machine-readable as `b_catalogue()` beside this crate's conformance suite, guarded by the drift test in `conformance_catalogue.rs`: a renamed or deleted suite fails the build rather than leaving the column below pointing at nothing. **B4** is checked there too, from the crate's manifest — the data path runs no consensus because no consensus engine is in its dependency graph to run, and a `granary` dependency appearing here would fail the test rather than quietly invalidating the claim.

| # | Invariant | Defined in | Verified by |
|---|---|---|---|
| **B1** | **Address integrity.** `get(ns, id)` returns bytes whose BLAKE3 hash equals `id`, or an error; it never returns wrong or corrupt bytes. Verification is on the read path, after any network transfer. | §2, §4 | `a_corrupt_blob_is_detected_and_never_returned`, `a_tampered_copy_falls_through_to_a_good_owner` |
| **B2** | **Idempotent, dedup'd put.** Equal content under the same namespace yields one stored copy; a `put` of already-present content writes nothing new and re-acknowledges. | §2, §5 | `putting_the_same_bytes_twice_stores_once`, `equal_content_converges_to_one_id_across_nodes` |
| **B3** | **Durability target.** A `put` is acknowledged only once at least W copies are stored (one on `Local`); the blob then survives the loss of any R − W owners. | §5.2 | `a_blob_put_on_one_node_is_gettable_from_every_node`, `a_put_below_write_quorum_is_refused` |
| **B4** | **No consensus on the data path.** The store runs no election, term, agreement round, or write-time read-repair; concurrent writers of the same content do not coordinate and do not fork. | §4 | structural: the crate depends on `actor-cluster`, not `granary`, so no consensus group exists to run; plus the B2 convergence tests |
...
- **Compaction helper.** A reusable routine for the consumer pattern of reclaiming individual blobs within a live namespace: copy the still-referenced blobs from `ns` into a fresh `ns'`, redirect the consumer's references, then `delete_namespace(ns)`. The store stays free of reference knowledge; the helper packages the copy-and-swap.
- **Reference-aware per-blob delete.** A `delete(ns, id)` with refcount or consumer-declared roots, for a consumer that needs to drop a single blob without compacting. Not in v1; compaction covers the motivating cases.
- **Cross-namespace dedup.** An optional shared namespace (never deleted, or compacted on its own schedule) into which consumers place content they want two lifecycles to share, recovering the cross-namespace dedup §2 gives up, at the cost of that namespace needing its own reclamation story.
- **Range-verified streaming.** Independently verifying a byte range of one large blob against the BLAKE3 tree the `BlobId` already roots (the Bao encoding), so very large blobs need not be fetched whole to be verified (§2).
