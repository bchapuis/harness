...
`Manifest` is a `const`-constructible newtype over `&'static str`. The transport boundary passes the inner `&'static str` (§4.1); in the envelope it travels as an owned `String` (§7.1), matched against the registered manifests on receipt (§4.4).
...
impl<A: Actor> ActorRef<A> {
    ...
    /// Fire-and-forget. Errors only for transport failure, never for handler
    /// outcome; a full local mailbox awaits space (§6).
...
5. resign_id(id)                  // on stop/failure; A::stopped() runs; releases all resources
```
...
Steps 2 and 3 overlap in time: `spawn` is synchronous (§4.1), so `actor_ready` does not await the async `A::started`. Messages delivered after `actor_ready` queue in the mailbox while `started` runs; the normative ordering is *`started` completes before the first user message is processed*, not *`started` completes before `actor_ready`*.
...
1. Decode the envelope header → `recipient: ActorId`, `manifest: String` (matched against the registered manifests, §3.2), optional `correlation`.
...
4. **Backpressure.** Mailboxes are bounded. When one is full, the system MUST apply a defined policy: `await` until space frees, or fail the send with `CallError::MailboxFull`. It MUST NOT drop messages silently. The two policies are separate calls: `tell` awaits, `try_tell` (§3.3) returns `CallError::MailboxFull` at once (local target).
5. **Ordering guarantee.** Messages from a single sender to a single recipient arrive in send order (FIFO per directed pair). For a remote recipient this order is scoped to a single association (§7.2). The system guarantees no ordering across different senders. `tell` and `ask` from the same sender share this order.
...
- **No silent loss.** Every `ask` MUST terminate in a reply or a definite `CallError` (§14.1); the invariant catalogue enumerates the outcomes (§18.5 #1). A pending ask whose target node is declared `down` (§9.4) MUST complete with `Unreachable`; one whose target is confirmed `unreachable` (§10) MAY complete with `Unreachable` early — at-most-once delivery (above) makes that safe even if the node recovers; failing both, the ask's deadline (§14.2) bounds it.
- **Ordering** per directed sender→recipient pair, as in §6, holds end-to-end over a single association. Across an association loss and re-establishment, no order is defined between frames the old association had accepted and frames sent over the new one; a sender that needs ordering across that boundary re-synchronizes at the application level (an `ask` round-trip suffices).
...
- **joining**: handshake complete, not yet admitted to full participation. Used by the modes where admission is an in-cluster decision (leader-based and gossip-based, §9.4); static starts every member `up`, and in registry-based mode admission *is* the registry entry (§9.4.2).
...
| Who decides | **Static** | **Registry-based** | **Leader-based** | **Gossip-based** |
|---|---|---|---|---|
...
1. **Single writer, total order.** Every membership transition — `joining → up`, `drain`/`resume`, a graceful leave (committed at the departing node's request), anything `→ down` — MUST be a log entry committed through the current leader. The commit index is the authority stamp (§9.2): members apply committed entries in log order and converge on one sequence of membership states. A command offered to a non-leader is redirected or rejected, never applied.
   ...
   **A committed entry a member cannot decode MUST stop that member's control plane, never be skipped.** Members apply in log order, so an entry one member skips is one its peers apply: its view is then not a *prefix* of the log but a **gap** in it, holding the transitions after an admission or a downing it never saw, while it remains a full member of the cluster whose membership it has stopped tracking. Skipping leaves item 1's total order gated for everyone but that member. So a member that cannot decode a committed transition MUST stop applying transitions, permanently, and MUST NOT proceed as though it had applied one.

   **The transition payload carries its own revision** (`actor.membership`, [compatibility](compatibility-spec.md) §3). It rides `EntryPayload::App` inside the already-stamped `actor.raft.log`, whose revision covers the *envelope* and not it, so without a stamp of its own the two reasons a member stops here — a transition a later build defined, and a truncated or damaged payload — are one answer with one message, and they have opposite fixes: run the node on a build that accepts the entry, or repair a log. An unstamped payload is adopted as revision 1, because every transition committed before the stamp existed is still in some cluster's log and a build that refused them would refuse its own history. The stamp is worth its bytes here despite being the widest *relative* cost in the tree — a magic and a revision more than double a nine-byte payload — because the absolute cost is eight bytes on a record written once per membership change, and because a stamp must be on the bytes before the revision that needs it exists.

   It SHOULD keep replicating and acknowledging the log: the entry is durable either way, and a restart onto a build that can read it applies it, so refusing the append would cost availability and buy no safety. If it leads, it SHOULD hand leadership to a peer that can still apply (item 8) — a leader that cannot observe the effect of its own committed decisions will propose them again on every pass, forever. Its condition SHOULD be readable by an operator, for the reason item 2 gives about a voter that has stopped voting. And where the mode also disseminates stamped status by gossip (item 3), such a member SHOULD keep gossiping: a peer that applied the entry can still deliver its *outcome*, which is what bounds the damage of stopping — though only for transitions the stopped build can name, which by construction excludes the one that stopped it.
2. **Voters.** A configured, modest subset of members (typically 3 or 5) are **voters**; only they elect leaders and form quorums. Voters MUST persist their Raft state (term, vote, log) durably and MUST keep stable node identities across restarts. Changes to the voter set are themselves committed configuration entries (single-server change or joint consensus). A voter declared `down` MUST be removed from the voter set by a committed change; its replacement joins as a new member with a fresh `NodeId` (§9.1).
   ...
   **A voter that cannot persist MUST stop voting.** Every write to that durable state happens *before* the state it records is announced, so a write reported as durable that was not is how a voter grants a second vote in a term across a restart, or acknowledges an entry it cannot replay. The durability seam MUST therefore be able to report failure, and a failure MUST be permanent for that voter: it steps down, and thereafter grants no vote, starts no election, and acknowledges no append. It MUST NOT resume on a later success — nothing at this layer can establish that the volume recovered, and a log that resumed after a gap presents the records either side of it as contiguous.

   A failed voter SHOULD stay reachable and keep answering with its term, refusing explicitly, rather than falling silent: peers then elect around it at once instead of waiting out a timeout on every append, and its condition SHOULD be readable by an operator, since a node that has stopped voting is otherwise indistinguishable from one that is merely slow. In particular an implementation SHOULD NOT express this as a panic unless the runtime guarantees that a panic halts the whole node — where supervision restarts the faulted unit, or the fault kills one task, the node survives and stays counted in quorums with no durability behind it, which is the failure this rule exists to prevent.
...
8. **Leadership transfer (RECOMMENDED).** A group leader SHOULD be able to hand leadership to a named voter without waiting out an election timeout (Raft §3.10): it verifies that the target has replicated the leader's last entry, then invites it to stand for election immediately. The invitation MUST be refused by a target whose log is behind — such a target would lose the election it was invited to hold, so sending it costs a disrupted term and achieves nothing — and MUST be accepted only from the leader of the recipient's current term. Transfer is safe by construction and needs no new agreement: it starts an ordinary election, which still requires a quorum of votes, so it can move leadership but never split it, and initiating one does not itself end the leader's term. Its purpose is planned departure. Without it a node that leads many groups costs one full election timeout *per group* when it stops, which makes a rolling restart — the most frequent operation a production cluster performs — indistinguishable from that many simultaneous failures.
9. **Heartbeat coalescing (RECOMMENDED).** A heartbeat is an empty append, and a node leading `G` groups sends one to each of `R-1` followers every heartbeat interval: `G × (R-1)` frames per interval carrying no data. Where `G` grows with the deployment — granary runs one group per shard (grain §8.2), and shard count is its elasticity knob (grain §7.7) — an implementation SHOULD batch the empty appends bound for one peer into a single frame, and their replies likewise. The batching MUST be framing only: each heartbeat is dispatched to its own group exactly as a lone append would be, so no group's state machine can observe the difference. An implementation SHOULD also send the batches **before** applying the tick's per-group outputs, so heartbeat latency does not grow with `G` — a heartbeat delayed behind `G` units of work is what turns a well-packed node into a source of spurious elections, which is the failure this item exists to prevent.
10. **The first election is not the detector's business (RECOMMENDED).** The election timeout exists to notice a leader that *stopped*. A group that has never had one — no persisted term, no log, no snapshot — is detecting nothing, and an implementation SHOULD NOT make it wait out that timeout before its first campaign. This is not a micro-optimization: groups are created in sequence (a shard's group only exists once the allocation naming it commits), so a cold cluster pays the timeout once per stage, serially, before it can serve anything. A group that *does* hold persisted state keeps the full timeout — a voter rejoining a healthy group has no business campaigning early.

    Two constraints bound how eager that first campaign may be. It MUST NOT begin before an existing leader could have been heard from — at least one heartbeat interval — because groups are created on each node as the allocation reaches it, not simultaneously, and a node building its copy late is "pristine" while the group already has a leader; campaigning first leaves it a candidate with no leader to route to, which is a stall the caller pays and not merely a wasted vote. And the delay SHOULD be staggered per voter, deterministically, so a cold cluster elects in sequence rather than tying and re-arming. Deriving that stagger from `(group, node)` rather than node identity alone also keeps a single node from leading every group. A deterministic stagger also draws no entropy, which §18.1 requires of anything on this path: a draw here would shift the seeded stream and change the behaviour of every recorded seed.
...
3. **Coordinator.** Transitions that need a single actor — admitting `joining → up`, finalizing `leaving → down` — are performed by the **coordinator**: a deterministic *role*, not an election — it falls to the lowest-address `up`, reachable member of each node's converged view. The coordinator acts only on a **locally stable, fully-reachable view** (every live member it can see is `reachable`), so it never transitions members while its own view is in flux — with one exception: applying the downing policy to an `unreachable` member (#4), which cannot be gated on full reachability because the node being downed is unreachable by definition. Two nodes that transiently both consider themselves coordinator cannot conflict: their decisions merge through the lattice, since `up` is monotonic and `down` terminal (§9.2).
...
```rust
// via Ctx (§3.4):
fn watch<B: Actor>(&self, target: &ActorRef<B>) where A: Handler<Terminated>;
fn unwatch<B: Actor>(&self, target: &ActorRef<B>);
...
2. **Authentication.** The handshake SHOULD authenticate the peer and MAY enforce an allowlist of permitted node identities or clusters; a cluster secret prevents accidental cross-cluster association. The secret is carried **in** the handshake, so on an unencrypted association it is visible to anyone who can observe the traffic, and observing one handshake is enough to join the cluster as a full member. A cluster secret is therefore protection against *accident*, not against an adversary on the path; only transport security (item 1) provides the latter.
3. **Encryption is a stated choice, not a default.** A configuration MUST make a plaintext association something a deployment names explicitly, rather than what it gets by omitting the TLS material. The two are equally expressible either way, but only the first makes the decision — and its consequence for item 2's secret — visible where it is taken and at every review of that code. Plaintext remains correct on loopback, in tests, and on a network the operator controls end to end.
4. **Deserialization safety.** As in §5, the dispatch registry MUST instantiate only registered, allowlisted message types from network input. No path may lead from an incoming envelope to constructing an arbitrary type.
5. **Authorization.** A system MAY gate `deliver` per `(peer, actor, manifest)` through an optional `Authorizer` (`fn authorize(&self, peer: NodeId, recipient: &ActorId, manifest: &str) -> bool`), consulted before an envelope is dispatched. A denied message MUST be rejected as a system failure — never deserialized into the actor's type, so an unauthorized peer cannot trigger handler side effects. With no `Authorizer`, every message that clears the handshake is admitted.
...
1. **Seed-reproducibility.** Given the same seed, configuration, and workload, two runs MUST produce byte-identical event streams (§16). All nondeterminism MUST pass through the `Clock`, `Entropy`, and `Spawner` traits (§4.6), the `Transport` trait (§7), and — in registry-based mode — the registry client (§9.4.2).
...
- **Membership / control plane:** dropped or delayed pings, partitions, stale or replayed gossip, stale incarnations (§9 and §10); leader crashes and elections mid-transition (leader-based mode); a stalled, lagging, or unavailable registry sync (registry-based mode).
...
3. **Per-pair FIFO (§6).** Messages from one sender to one recipient are observed in send order (within one association, §6, §7.2), even under maximal reordering injection; no ordering is assumed across senders.
...
14. **Membership convergence (§9.2, §9.4).** Once faults cease and partitions heal, all `up` members converge on one membership set within bounded logical time — by anti-entropy (gossip-based), registry sync (registry-based), or log replication (leader-based).
15. **`down` is terminal (§9.1).** A node observed `down` never reappears `up` under the same incarnation.
16. **Partition tolerance (§9.4).** Under the default downing policy, a partition alone never moves a member to `down`, only to `unreachable`. **Registry-based** mode holds this unconditionally: its detector is observe-only, so only the registry ever declares `down`. **Leader-based** mode holds it for any side without quorum, which can commit no transition at all (#22).
...
22. **Quorum-gated control plane (§9.4.3).** In leader-based mode, every membership transition is a quorum-committed log entry, applied in log order; at most one leader per term issues them, and a partition side lacking a quorum of voters commits none — a minority can never evict the majority. Applying *in log order* admits no skipping: a member that cannot decode a committed transition stops applying rather than stepping over it, so each member's view is always a prefix of the committed log and never a gap in it.
